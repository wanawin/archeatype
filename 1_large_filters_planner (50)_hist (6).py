...
# ===================== HFR ADDITIVE SECTION (Delegates to base evaluator if present) =====================
# Purely additive. Does not modify any existing code above.
import re
import types
import pandas as pd
...
def HFR_sum_of_digits(digs): return sum(digs)
def HFR_parity_counts(digs):
    ev = sum(1 for d in digs if d % 2 == 0)
    return ev, len(digs) - ev
def HFR_high_low_counts(digs):
    lo = sum(1 for d in digs if d in {0,1,2,3,4})
    hi = len(digs) - lo
    return hi, lo
HFR_V_TRAC_GROUPS = {0:1,5:1,1:2,6:2,2:3,7:3,3:4,8:4,4:5,9:5}
def HFR_vtrac_groups(digs): return [HFR_V_TRAC_GROUPS[d] for d in digs]
def HFR_spread(digs): return max(digs) - min(digs) if digs else 0
...
def HFR_seed_profile(digs):
    return {
        "sum": HFR_sum_of_digits(digs),
        "sum_category": HFR_sum_category(HFR_sum_of_digits(digs)),
        "even": HFR_parity_counts(digs)[0],
        "odd": HFR_parity_counts(digs)[1],
        "high": HFR_high_low_counts(digs)[0],
        "low": HFR_high_low_counts(digs)[1],
        "vtrac": HFR_vtrac_groups(digs),
        "spread": HFR_spread(digs),
    }
...
def HFR_build_combo_context(combo_digits, seed_digits):
    d0,d1,d2,d3,d4 = (combo_digits + [None]*5)[:5]
    even_count = HFR_parity_counts(combo_digits)[0]
    odd_count = HFR_parity_counts(combo_digits)[1]
    high_count = HFR_high_low_counts(combo_digits)[0]
    low_count  = HFR_high_low_counts(combo_digits)[1]
    ctx = {
        # canonical
        "combo_digits": combo_digits, "seed_digits": seed_digits,
        "d0": d0, "d1": d1, "d2": d2, "d3": d3, "d4": d4,
        "combo_sum": HFR_sum_of_digits(combo_digits),
        "combo_spread": HFR_spread(combo_digits),
        "even_count": even_count, "odd_count": odd_count,
        "high_count": high_count, "low_count": low_count,
        "vtrac": HFR_vtrac_groups(combo_digits),
        # aliases seen in legacy filters
...
def HFR_get_base_evaluator():
    # Search for common evaluator names defined in base app
    candidates = [
        "evaluate_filter", "evaluate_expression", "apply_filter_expression",
...
def HFR_safe_eval_expression(expr: str, context: dict) -> bool:
    ...
    return bool(eval(expr, {"__builtins__": {}}, {**context, **SAFE}))

def HFR_eval_with_base_or_fallback(base_fn, expr, combo_digits, seed_digits):
    ctx = HFR_build_combo_context(combo_digits, seed_digits)
    # Try common signatures against base evaluator
    if base_fn is not None:
        try:
            return bool(base_fn(expr, ctx))          # (expr, context)
        except Exception:
            try:
                return bool(base_fn(expr, combo_digits, seed_digits))  # (expr, combo, seed)
            except Exception:
                try:
                    return bool(base_fn(ctx, expr))  # (context, expr)
                except Exception:
                    pass
    # Fallback to safe eval using our context
    return HFR_safe_eval_expression(expr, ctx)
...
def HFR_render():
    ...
    st.markdown("---")
    st.header("Historical Safety Recommender (delegates to base evaluator if present)")

    c1, c2 = st.columns(2)
    with c1:
        hist_file = st.file_uploader(
            "Upload history CSV/TXT (Result column OK; seeds auto-shifted)",
            type=["csv","txt"]
        )
        reverse_hint = st.checkbox("History file is reverse chronological (newest first)", value=True)
    with c2:
        filt_file = st.file_uploader("Upload Filters CSV (requires id + expression)", type=["csv"])

    seed_input = st.text_input("Current seed (5 digits)", "27500")
    seed_now = HFR_digits_from_str(seed_input)
    if len(seed_now) != 5:
        st.error("Seed must be 5 digits."); return
    prof_now = HFR_seed_profile(seed_now)

    with st.expander("Similarity & scoring", expanded=False):
        ...
        K = st.slider("Top-K neighbors", 10, 500, 200, 5)
        thr = st.slider("Minimum similarity", 0.0, 8.0, 3.0, 0.1)
        count_errors_as_elims = st.checkbox("Count expression errors as eliminations", value=False,
                                            help="OFF recommended until expressions stabilize.")
        true_means_keep = st.checkbox("Expression TRUE means: KEEP (not eliminate)", value=False)

    st.markdown("#### Filter scope (optional)")
    c3, c4 = st.columns(2)
    with c3:
        limit_to_ids = st.checkbox("Limit to pasted Filter IDs", value=False)
        pasted_ids = st.text_area("Paste IDs (comma/space/newline separated)", "", height=100)
    with c4:
        use_enabled_col = st.checkbox("Use only enabled==True (if present)", value=False)
        ids_from_csv = st.file_uploader("Or upload an IDs CSV (with 'id' column)", type=["csv"], key="ids_csv")

    run = st.button("Run Historical Safety")
    if not run: return
    if not hist_file: st.error("Upload a history file."); return
    if not filt_file: st.error("Upload a filters CSV."); return
    ...
    try:
        ...
        st.error(f"Could not read history: {e}"); return
    ...
    if seed_col is None and win_col is None:
        if len(dfh.columns) == 1:
            win_col = list(dfh.columns)[0]
            dfh = dfh.rename(columns={win_col:"Result"}); win_col = "Result"
        else:
            win_col = lowmap.get("result") or lowmap.get("draw")
    if win_col is None:
        st.error(f"History must include a results column (e.g., 'Result'). Found: {list(dfh.columns)}"); return

    if seed_col is None:
        series = dfh[win_col].astype(str).tolist()
        if len(series) < 2: st.error("History needs at least 2 rows."); return
        if reverse_hint:
            seeds = series[1:]; wins = series[:-1]   # newest->oldest
        else:
            seeds = series[:-1]; wins = series[1:]   # oldest->newest
        dfh = pd.DataFrame({"seed":seeds,"winner":wins})
    else:
        dfh = dfh[[seed_col, win_col]].rename(columns={seed_col:"seed", win_col:"winner"})

    dfh["seed_digits"] = dfh["seed"].apply(HFR_digits_from_str)
    dfh["winner_digits"] = dfh["winner"].apply(HFR_digits_from_str)
    dfh = dfh[dfh["seed_digits"].apply(len)==5]
    dfh = dfh[dfh["winner_digits"].apply(len)==5].reset_index(drop=True)

    # Preview alignment (first 20)
    with st.expander("Seed/Winner preview (first 20 after shifting)", expanded=False):
        st.dataframe(dfh.head(20), hide_index=True, use_container_width=True)

    # Similarity
    dfh["p_sum"] = dfh["seed_digits"].apply(HFR_sum_of_digits)
    dfh["p_sumcat"] = dfh["p_sum"].apply(HFR_sum_category)
    dfh["p_even"] = dfh["seed_digits"].apply(lambda d: HFR_parity_counts(d)[0])
    dfh["p_odd"]  = dfh["seed_digits"].apply(lambda d: HFR_parity_counts(d)[1])
    dfh["p_high"] = dfh["seed_digits"].apply(lambda d: HFR_high_low_counts(d)[0])
    dfh["p_low"]  = dfh["seed_digits"].apply(lambda d: HFR_high_low_counts(d)[1])
    dfh["p_vtr"]  = dfh["seed_digits"].apply(HFR_vtrac_groups)
    dfh["p_spread"] = dfh["seed_digits"].apply(HFR_spread)

    sims = []
    for _, row in dfh.iterrows():
        ph = {"sum":row["p_sum"],"sum_category":row["p_sumcat"],"even":row["p_even"],"odd":row["p_odd"],
              "high":row["p_high"],"low":row["p_low"],"vtrac":row["p_vtr"],"spread":row["p_spread"]}
        sims.append(HFR_similarity_score(prof_now, ph, weights))
    dfh["similarity"] = sims
    nbrs = dfh[dfh["similarity"]>=thr].sort_values("similarity", ascending=False).head(K)
    st.info(f"Neighbors used: {len(nbrs)}")
    if nbrs.empty: st.warning("No neighbors—lower similarity or raise K."); return

    # Filters
    try:
        dff = pd.read_csv(filt_file)
    except Exception as e:
        st.error(f"Could not read filters CSV: {e}"); return
    ...
    if limit_to_ids:
        if pasted_ids.strip():
            ...
            include_ids.update(t.strip() for t in tokens if t.strip())
        if ids_from_csv is not None:
            try:
                df_ids = pd.read_csv(ids_from_csv)
                lm = {c.lower(): c for c in df_ids.columns}
                idname = lm.get("id")
                if idname:
                    include_ids.update(df_ids[idname].astype(str).str.strip().tolist())
            except Exception as e:
                st.warning(f"IDs CSV read error: {e}")
        if include_ids:
            dff = dff[dff["id"].astype(str).isin(include_ids)].copy()
        else:
            st.warning("Limit to IDs was ON but no IDs parsed; evaluating all filters.")

    # Evaluate using base evaluator if available
    base_fn = HFR_get_base_evaluator()

    results = []
    for _, frow in dff.iterrows():
        fid, fname, expr = str(frow["id"]), str(frow["name"]), str(frow["expression"]).strip()
        if not expr: continue
        elim = err = valid = 0
        for _, r in nbrs.iterrows():
            seed_d = r["seed_digits"]; combo_d = r["winner_digits"]  # use actual next draw as the candidate combo
            try:
                out = HFR_eval_with_base_or_fallback(base_fn, expr, combo_d, seed_d)
                # Interpret semantics: True means eliminate by default; flip if user says True=Keep
                out = (not out) if true_means_keep else bool(out)
                valid += 1
                if out: elim += 1
            except Exception:
                err += 1
                if count_errors_as_elims:
                    elim += 1
        denom = valid if not count_errors_as_elims else (valid + err)
        elim_rate = (elim/denom) if denom else 0.0
        safety = 1.0 - elim_rate if denom else 0.0
        results.append({
            "id": fid, "name": fname,
            "valid_cases": valid, "error_cases": err,
            "elimination_rate_%": round(elim_rate*100, 2),
            "historical_safety_%": round(safety*100, 2)
        })

    if not results:
        st.error("No evaluable filters found."); return

    res = pd.DataFrame(results).sort_values(["historical_safety_%","valid_cases"], ascending=[False,False]).reset_index(drop=True)
    st.subheader("Recommended Filters — Historical Safety First (0–100%)")
