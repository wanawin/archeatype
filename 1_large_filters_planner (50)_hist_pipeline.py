...
# ===================== HFR ADDITIVE SECTION (Two‑Step Pipeline) =====================
# Base app is not modified. This section is appended and namespaced.
import re, types
import pandas as pd
...
# ---------- Helpers (namespaced) ----------
def HFRP_digits_from_str(s: str):
    s = ''.join(ch for ch in str(s) if ch.isdigit())
    return [int(ch) for ch in s]

def HFRP_sum_of_digits(digs): return sum(digs)
def HFRP_parity_counts(digs):
    ev = sum(1 for d in digs if d % 2 == 0); return ev, len(digs) - ev
def HFRP_high_low_counts(digs):
    lo = sum(1 for d in digs if d in {0,1,2,3,4}); hi = len(digs) - lo; return hi, lo
HFRP_V_TRAC_GROUPS = {0:1,5:1,1:2,6:2,2:3,7:3,3:4,8:4,4:5,9:5}
def HFRP_vtrac_groups(digs): return [HFRP_V_TRAC_GROUPS[d] for d in digs]
def HFRP_spread(digs): return max(digs) - min(digs) if digs else 0
def HFRP_sum_category(total: int) -> str:
    if total <= 15: return 'Very Low'
    if total <= 24: return 'Low'
    if total <= 33: return 'Mid'
    return 'High'
def HFRP_seed_profile(digs):
    return {
        "sum": HFRP_sum_of_digits(digs),
        "sum_category": HFRP_sum_category(HFRP_sum_of_digits(digs)),
        "even": HFRP_parity_counts(digs)[0],
        "odd": HFRP_parity_counts(digs)[1],
        "high": HFRP_high_low_counts(digs)[0],
        "low": HFRP_high_low_counts(digs)[1],
        "vtrac": HFRP_vtrac_groups(digs),
        "spread": HFRP_spread(digs),
    }
def HFRP_similarity_score(p_now: dict, p_hist: dict, weights=None):
    if weights is None:
...
# Base-evaluator delegation
def HFRP_get_base_evaluator():
    g = globals()
    for name in ["evaluate_filter","evaluate_expression","apply_filter_expression",
                 "passes_filter","filter_eval","filter_applies","eliminates_combo"]:
        fn = g.get(name)
        import types
...
def HFRP_build_combo_context(combo_digits, seed_digits):
    d0,d1,d2,d3,d4 = (combo_digits + [None]*5)[:5]
    even_count = HFRP_parity_counts(combo_digits)[0]
    odd_count = HFRP_parity_counts(combo_digits)[1]
    high_count = HFRP_high_low_counts(combo_digits)[0]
    low_count  = HFRP_high_low_counts(combo_digits)[1]
    return {
        # canonical
        "combo_digits": combo_digits, "seed_digits": seed_digits,
        "d0": d0, "d1": d1, "d2": d2, "d3": d3, "d4": d4,
        "combo_sum": HFRP_sum_of_digits(combo_digits),
        "combo_spread": HFRP_spread(combo_digits),
        "even_count": even_count, "odd_count": odd_count,
        "high_count": high_count, "low_count": low_count,
        "vtrac": HFRP_vtrac_groups(combo_digits),
        # aliases (legacy friendly)
...
def HFRP_eval_strict_bool(base_fn, expr_text, combo_digits, seed_digits):
    # Returns (is_bool, value_bool, error_code or None)
    ctx = HFRP_build_combo_context(combo_digits, seed_digits)
    if base_fn is not None:
        for sig in [(expr_text, ctx), (expr_text, combo_digits, seed_digits), (ctx, expr_text)]:
            try:
                val = base_fn(*sig)
                if isinstance(val, bool): return True, val, None
    ...
    try:
        ...
        val = eval(compiled, {"__builtins__": {}}, {**ctx, **SAFE_BUILTINS})
        if isinstance(val, bool): return True, val, None
        return False, False, "non_boolean"
    except Exception:
        return False, False, "exception"

# ---------- UI: Two-step pipeline ----------
def HFRP_render():
    import streamlit as st

    st.markdown("---")
    st.header("Historical Safety — Two‑Step Pipeline (Additive)")

    # Step 1: Similar seeds
    st.subheader("Step 1 · Find similar seeds")
    c1, c2 = st.columns(2)
    with c1:
        hist_file = st.file_uploader("Upload history CSV/TXT", type=["csv","txt"], key="hfrp_hist")
        reverse_hint = st.checkbox("History is reverse chronological (newest first)", value=True, key="hfrp_rev")
    with c2:
        seed_input = st.text_input("Current seed (5 digits)", "27500", key="hfrp_seed")
        seed_now = HFRP_digits_from_str(seed_input)
        if len(seed_now) != 5:
            st.error("Seed must be 5 digits."); return
        prof_now = HFRP_seed_profile(seed_now)

    with st.expander("Similarity settings", expanded=False):
        w_sum = st.slider("Weight: Sum", 0.0, 3.0, 1.0, 0.1, key="hfrp_wsum")
        w_sumcat = st.slider("Weight: Sum Category", 0.0, 3.0, 1.5, 0.1, key="hfrp_wsumcat")
        w_even = st.slider("Weight: Even Count", 0.0, 3.0, 0.6, 0.1, key="hfrp_weven")
        w_odd = st.slider("Weight: Odd Count", 0.0, 3.0, 0.6, 0.1, key="hfrp_wodd")
        w_high = st.slider("Weight: High Count", 0.0, 3.0, 0.8, 0.1, key="hfrp_whigh")
        w_low  = st.slider("Weight: Low Count", 0.0, 3.0, 0.8, 0.1, key="hfrp_wlow")
        w_spread = st.slider("Weight: Spread", 0.0, 3.0, 0.8, 0.1, key="hfrp_wspread")
        w_vtrac = st.slider("Weight: V-Trac Overlap", 0.0, 3.0, 1.2, 0.1, key="hfrp_wvtrac")
        weights = {"sum":w_sum,"sum_category":w_sumcat,"even":w_even,"odd":w_odd,"high":w_high,"low":w_low,"spread":w_spread,"vtrac":w_vtrac}
        K  = st.slider("Top-K neighbors", 10, 500, 200, 5, key="hfrp_k")
        thr = st.slider("Minimum similarity", 0.0, 8.0, 3.0, 0.1, key="hfrp_thr")

    if st.button("Compute similar seeds", key="hfrp_step1"):
        if not hist_file:
            st.error("Upload a history file to run Step 1."); st.stop()
        ...
        try:
            ...
            st.error(f"History read error: {e}"); st.stop()
        ...
        if win_col is None:
            st.error("History must include a results column (e.g., 'Result')."); st.stop()

        if seed_col is None:
            series = dfh[win_col].astype(str).tolist()
            if len(series) < 2: st.error("Need at least 2 rows."); st.stop()
            if reverse_hint:
        ...
        dfh["seed_digits"] = dfh["seed"].apply(HFRP_digits_from_str)
        dfh["winner_digits"] = dfh["winner"].apply(HFRP_digits_from_str)
        dfh = dfh[dfh["seed_digits"].apply(len)==5]
        dfh = dfh[dfh["winner_digits"].apply(len)==5].reset_index(drop=True)

        # Similarity
        dfh["p_sum"] = dfh["seed_digits"].apply(HFRP_sum_of_digits)
        dfh["p_sumcat"] = dfh["p_sum"].apply(HFRP_sum_category)
        dfh["p_even"] = dfh["seed_digits"].apply(lambda d: HFRP_parity_counts(d)[0])
        dfh["p_odd"]  = dfh["seed_digits"].apply(lambda d: HFRP_parity_counts(d)[1])
        dfh["p_high"] = dfh["seed_digits"].apply(lambda d: HFRP_high_low_counts(d)[0])
        dfh["p_low"]  = dfh["seed_digits"].apply(lambda d: HFRP_high_low_counts(d)[1])
        dfh["p_vtr"]  = dfh["seed_digits"].apply(HFRP_vtrac_groups)
        dfh["p_spr"]  = dfh["seed_digits"].apply(HFRP_spread)

        prof_now = {"sum":HFRP_sum_of_digits(seed_now),"sum_category":HFRP_sum_category(HFRP_sum_of_digits(seed_now)),
                    "even":HFRP_parity_counts(seed_now)[0],"odd":HFRP_parity_counts(seed_now)[1],
                    "high":HFRP_high_low_counts(seed_now)[0],"low":HFRP_high_low_counts(seed_now)[1],
                    "vtrac":HFRP_vtrac_groups(seed_now),"spread":HFRP_spread(seed_now)}

        dfh["similarity"] = [
            HFRP_similarity_score(prof_now, {"sum":r.p_sum,"sum_category":r.p_sumcat,"even":r.p_even,"odd":r.p_odd,
        ...
        st.session_state["HFRP_neighbors"] = nbrs[["seed","winner","seed_digits","winner_digits","similarity"]].copy()

        st.subheader("Neighbors preview")
        st.dataframe(st.session_state["HFRP_neighbors"].head(50), hide_index=True, use_container_width=True)

        # Allow download of winners for external validation
        wins_df = st.session_state["HFRP_neighbors"][["winner"]].rename(columns={"winner":"winner_draw"}).copy()
    ...
    # Step 2: Evaluate filters on locked winners only
    st.subheader("Step 2 · Run filters on locked winners only")
    filt_file = st.file_uploader("Upload Filters CSV (id + expression required)", type=["csv"], key="hfrp_filters")
    use_enabled_col = st.checkbox("Use only enabled==True (if present)", value=False, key="hfrp_enabled")
    limit_to_ids = st.checkbox("Limit to pasted Filter IDs", value=False, key="hfrp_limit")
    pasted_ids = st.text_area("Paste IDs (comma/space/newline separated)", "", height=100, key="hfrp_ids")

    if st.button("Evaluate filters on locked winners", key="hfrp_step2"):
        if "HFRP_neighbors" not in st.session_state:
            st.error("You must run Step 1 first to lock a neighbor set."); st.stop()
        if not filt_file:
            st.error("Upload a filters CSV."); st.stop()

        nbrs = st.session_state["HFRP_neighbors"].copy()

        try:
            dff = pd.read_csv(filt_file)
        except Exception as e:
            st.error(f"Filters read error: {e}"); st.stop()
        ...
        if id_col is None or expr_col is None:
            st.error("Filters CSV must include 'id' and 'expression'."); st.stop()
        ...
        if use_enabled_col and ("enabled" in dff.columns):
            dff = dff[dff["enabled"].astype(str).str.lower().isin(["true","1","yes","y","t"])].copy()

        if limit_to_ids and pasted_ids.strip():
            tokens = re.split(r"[,\s]+", pasted_ids.strip())
            include_ids = [t.strip() for t in tokens if t.strip()]
            dff = dff[dff["id"].astype(str).isin(include_ids)].copy()

        base_name, base_fn = HFRP_get_base_evaluator()

        results = []
        for _, frow in dff.iterrows():
            fid, fname, expr_text = str(frow["id"]), str(frow["name"]), str(frow["expression"]).strip()
            if not expr_text: continue
            valid = err_nonbool = err_exc = elim = 0
            for _, r in nbrs.iterrows():
                seed_d = r["seed_digits"]; combo_d = r["winner_digits"]
                ok, val, err = HFRP_eval_strict_bool(base_fn, expr_text, combo_d, seed_d)
                if err is None:
                    valid += 1
                    if val is True: elim += 1  # True => eliminate
        ...
        if not results:
            st.error("No evaluable filters found."); st.stop()

        out = pd.DataFrame(results).sort_values(["historical_safety_%","valid_cases"], ascending=[False,False]).reset_index(drop=True)
        st.success(f"Evaluated {len(out)} filters against {len(nbrs)} winners from similar seeds.")
...
try:
    ...
    with st.sidebar:
        st.markdown("---")
        st.markdown("### Historical Safety (Additive)")
        _HFRP_SHOW = st.checkbox("Show Historical Safety — Two‑Step Pipeline", value=False)
    if _HFRP_SHOW:
        HFRP_render()
except Exception:
    pass
