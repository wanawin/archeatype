...
# ===================== ADDITIVE: Historical Safety — Two-Step (Same Page) =====================
# Uses the base app's normalize_expr, SAFE_BUILTINS, digits_of, make_base_env, combo_env.
import streamlit as st
import pandas as pd
import re
...
def _hsp_digits(s: str):
    return digits_of(s)

def _hsp_seed_profile(digs):
    def parity_counts(d): 
        ev = sum(1 for x in d if x % 2 == 0); return ev, len(d)-ev
    def high_low_counts(d):
        lo = sum(1 for x in d if x <= 4); hi = len(d)-lo; return hi, lo
    V = {0:1,5:1,1:2,6:2,2:3,7:3,3:4,8:4,4:5,9:5}
    def vtrac_groups(d): return [V.get(x) for x in d]
    def spread(d): return max(d)-min(d) if d else 0
    s = sum(digs)
    sum_category = 'Very Low' if s<=15 else ('Low' if s<=24 else ('Mid' if s<=33 else 'High'))
    even, odd = parity_counts(digs)
    high, low = high_low_counts(digs)
    return {
        "sum": s, "sum_category": sum_category,
        "even": even, "odd": odd, "high": high, "low": low,
        "vtrac": vtrac_groups(digs), "spread": spread(digs)
    }

def _hsp_similarity(p_now, p_hist, w=None):
...
def _hsp_compile_expr(expr_text: str):
    # Reuse base normalizer
...
def _hsp_eval_bool_on_winner(expr_code, seed_digits, winner_digits, base_env):
    # Build per-combo env with base's combo_env
    env = combo_env(base_env, "".join(str(d) for d in winner_digits))
    globs = {"__builtins__": {}}; globs.update(env)
    try:
        val = eval(expr_code, globs, env)
...
def _render_two_step_same_page():
    with st.sidebar:
        st.markdown("---")
        show = st.checkbox("Show Historical Safety — Two-Step (same page)", value=True)
    if not show: return

    st.markdown("## Historical Safety — Two-Step (same page)")

    c1, c2 = st.columns(2)
    with c1:
        hist_file = st.file_uploader("History CSV/TXT", type=["csv","txt"], key="h2_hist")
        reverse_hint = st.checkbox("History is reverse chronological (newest first)", value=True, key="h2_rev")
    with c2:
        seed_input = st.text_input("Current seed (5 digits)", "", key="h2_seed")

    if not seed_input or len(_hsp_digits(seed_input)) != 5:
        st.warning("Enter a 5-digit seed to proceed.")
        return
    seed_now = _hsp_digits(seed_input)
    prof_now = _hsp_seed_profile(seed_now)

    with st.expander("Similarity settings", expanded=False):
        w_sum = st.slider("Weight: Sum", 0.0, 3.0, 1.0, 0.1, key="h2_wsum")
        w_sumcat = st.slider("Weight: Sum Category", 0.0, 3.0, 1.5, 0.1, key="h2_wsumcat")
        w_even = st.slider("Weight: Even", 0.0, 3.0, 0.6, 0.1, key="h2_wev")
        w_odd  = st.slider("Weight: Odd", 0.0, 3.0, 0.6, 0.1, key="h2_wod")
        w_high = st.slider("Weight: High", 0.0, 3.0, 0.8, 0.1, key="h2_whi")
        w_low  = st.slider("Weight: Low", 0.0, 3.0, 0.8, 0.1, key="h2_wlo")
        w_spread = st.slider("Weight: Spread", 0.0, 3.0, 0.8, 0.1, key="h2_wsp")
        w_vtrac = st.slider("Weight: V-Trac Overlap", 0.0, 3.0, 1.2, 0.1, key="h2_wvtr")
        weights = {"sum":w_sum,"sum_category":w_sumcat,"even":w_even,"odd":w_odd,"high":w_high,"low":w_low,"spread":w_spread,"vtrac":w_vtrac}
        K  = st.slider("Top-K neighbors", 10, 500, 200, 5, key="h2_k")
        thr = st.slider("Minimum similarity", 0.0, 8.0, 3.0, 0.1, key="h2_thr")

    if st.button("Step 1 — Lock similar seeds", key="h2_step1"):
        if not hist_file:
            st.error("Upload history to continue.")
            st.stop()
        try:
        ...
        dfh["similarity"] = [
            _hsp_similarity(prof_now, {
                "sum":r.p_sum,"sum_category":r.p_sumcat,"even":r.p_even,"odd":r.p_odd,
                "high":r.p_high,"low":r.p_low,"vtrac":r.p_vtr,"spread":r.p_spread
            }, weights) for _, r in dfh.iterrows()
        ]
        nbrs = dfh[dfh["similarity"]>=thr].sort_values("similarity", ascending=False).head(K)
        if nbrs.empty:
            st.warning("No neighbors — relax similarity or raise Top-K.")
            st.stop()

        st.session_state["H2_NEIGHBORS"] = nbrs[["seed","winner","seed_digits","winner_digits","similarity"]].copy()
        st.success(f"Locked {len(st.session_state['H2_NEIGHBORS'])} similar seeds.")
        st.dataframe(st.session_state["H2_NEIGHBORS"].head(50), hide_index=True, use_container_width=True)

        wins_df = st.session_state["H2_NEIGHBORS"][["winner"]].rename(columns={"winner":"winner_draw"}).copy()
    ...
    filt_file = st.file_uploader("Upload Filters CSV (id + expression required)", type=["csv"], key="h2_filters")
    limit_to_ids = st.checkbox("Limit to pasted Filter IDs", value=False, key="h2_limit_ids")
    pasted_ids = st.text_area("Paste IDs (comma/space/newline separated)", "", height=100, key="h2_ids")

    if st.button("Evaluate on locked winners", key="h2_step2"):
        if "H2_NEIGHBORS" not in st.session_state:
        ...
        # Compile expressions using the base normalizer
        comp_rows = []
        for _, r in dff.iterrows():
            code, norm = _hsp_compile_expr(str(r["expression"]))
            comp_rows.append({"id":str(r["id"]), "name":str(r["name"]), "expr_code":code, "expr_norm":norm})
        comp_df = pd.DataFrame(comp_rows)

        # Build base environment ONCE for the current seed (prev seeds empty for historical step)
        base_env = make_base_env(
            "".join(str(d) for d in seed_now), "", "", "",
            st.session_state.get("hot_digits", []),
            st.session_state.get("cold_digits", []),
            st.session_state.get("due_digits", []),
        )

        nbrs = st.session_state["H2_NEIGHBORS"]
        results = []
        for _, fr in comp_df.iterrows():
            fid, fname, expr_code = fr["id"], fr["name"], fr["expr_code"]
            valid = err_nb = err_ex = elim = 0
            for _, nr in nbrs.iterrows():
                seed_d = nr["seed_digits"]; win_d = nr["winner_digits"]
                is_bool, val, err = _hsp_eval_bool_on_winner(expr_code, seed_d, win_d, base_env)
                if err is None:
                    valid += 1
            ...
            results.append({
                "id": fid, "name": fname,
                "valid_cases": valid,
                "error_cases_nonbool": err_nb,
                "error_cases_exceptions": err_ex,
                "historical_safety_%": round(safety, 2)
            })

        out = pd.DataFrame(results).sort_values(["historical_safety_%","valid_cases"], ascending=[False,False]).reset_index(drop=True)
        st.success(f"Evaluated {len(out)} filters against {len(nbrs)} neighbors' winners.")
...
# Hook the section into the same page without altering existing widgets
try:
    _render_two_step_same_page()
except Exception as _e:
    import traceback, sys
    st = sys.modules.get("streamlit")
    if st:
        st.warning(f"Historical section failed to render: {_e}")
