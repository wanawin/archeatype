...
# ================== ADDITIVE: Historical Safety — Two-Step (V6: weighting + overall stats) ==================
import streamlit as st
import pandas as pd
import re
...
def _h6_digits(s: str): return digits_of(s)

def _h6_profile(digs):
    ...
    s=sum(digs)
    sum_bucket = 'Very Low' if s<=15 else ('Low' if s<=24 else ('Mid' if s<=33 else 'High'))
    ev,od=pc(digs); hi,lo=hl(digs)
    spr=(max(digs)-min(digs)) if digs else 0
    return {"sum":s,"sum_bucket":sum_bucket,"even":ev,"odd":od,"high":hi,"low":lo,"vtrac":v(digs),"spread":spr}

def _h6_similarity(p0, p1, weights, gamma=1.0):
    w = weights
    s = 0.0; total = 0.0
    total += w["sum_bucket"]; s += w["sum_bucket"] * (1.0 if p0["sum_bucket"]==p1["sum_bucket"] else 0.0)
    for k,cap in [("even",5.0),("odd",5.0),("high",5.0),("low",5.0)]:
        total += w[k]
        s += w[k] * (1.0 - min(abs(p0[k]-p1[k])/cap, 1.0))
    total += w["spread"]; s += w["spread"] * (1.0 - min(abs(p0["spread"]-p1["spread"])/9.0, 1.0))
    nv, hv = Counter(p0["vtrac"]), Counter(p1["vtrac"])
    overlap = sum(min(nv[t], hv[t]) for t in nv.keys() & hv.keys())/5.0
    total += w["vtrac"]; s += w["vtrac"] * overlap
    raw = (s/total) if total>0 else 0.0
    return raw**gamma

def _beta_shrink(elim_w, total_w, a0, b0):
    return (a0 + elim_w) / (a0 + b0 + total_w) if (a0 + b0 + total_w) > 0 else 0.0

def _render_two_step_v6():
    with st.sidebar:
        st.markdown("---")
        show = st.checkbox("Historical Safety — Two-Step (V6: weighted + overall)", value=True)
    if not show: return

    st.markdown("## Historical Safety — Two-Step (V6)")

    c1,c2 = st.columns(2)
    with c1:
        hist_file = st.file_uploader("History CSV/TXT", type=["csv","txt"], key="h6_hist")
        reverse = st.checkbox("History is reverse chronological (newest first)", value=True, key="h6_rev")
    with c2:
        seed_txt = st.text_input("Current seed (5 digits) — used only to find neighbors", "", key="h6_seed")

    if not seed_txt or len(_h6_digits(seed_txt))!=5:
        st.warning("Enter a 5-digit seed to proceed."); return
    current_seed = _h6_digits(seed_txt)
    p0 = _h6_profile(current_seed)

    with st.expander("Similarity & weighting", expanded=False):
        colA, colB, colC = st.columns(3)
        with colA:
            w_sum_bucket = st.slider("Weight: Sum bucket", 0.0, 3.0, 1.5, 0.1, key="h6_wsb")
            w_even = st.slider("Weight: Even", 0.0, 3.0, 0.6, 0.1, key="h6_wev")
            w_odd  = st.slider("Weight: Odd", 0.0, 3.0, 0.6, 0.1, key="h6_wod")
        with colB:
            w_high = st.slider("Weight: High", 0.0, 3.0, 0.8, 0.1, key="h6_whi")
            w_low  = st.slider("Weight: Low", 0.0, 3.0, 0.8, 0.1, key="h6_wlo")
            w_spread = st.slider("Weight: Spread", 0.0, 3.0, 0.8, 0.1, key="h6_wsp")
        with colC:
            w_vtrac = st.slider("Weight: V-Trac overlap", 0.0, 3.0, 1.2, 0.1, key="h6_wvt")
            gamma = st.slider("Similarity sharpness (gamma)", 1.0, 3.0, 1.6, 0.1, key="h6_gamma")
            topK  = st.slider("Top-K neighbors", 10, 500, 200, 5, key="h6_k")
        weights = {"sum_bucket":w_sum_bucket,"even":w_even,"odd":w_odd,"high":w_high,"low":w_low,"spread":w_spread,"vtrac":w_vtrac}

        st.markdown("**Shrinkage prior (stabilize small-sample rates):**")
        prior = st.selectbox("Prior", ["Laplace (a0=b0=1)", "None"], index=0, key="h6_prior")

    if st.button("Step 1 — Lock similar seeds", key="h6_s1"):
        if not hist_file:
        ...
        dfh["seed_digits"]=dfh["seed"].apply(_h6_digits)
        dfh["winner_digits"]=dfh["winner"].apply(_h6_digits)
        dfh=dfh[dfh["seed_digits"].apply(len)==5]
        dfh=dfh[dfh["winner_digits"].apply(len)==5].reset_index(drop=True)

        profs = dfh["seed_digits"].apply(_h6_profile)
        sim_scores = []
        for p in profs:
            sim_scores.append(_h6_similarity(p0, p, weights, gamma))
        dfh["similarity"] = sim_scores

        nbrs = dfh.sort_values("similarity", ascending=False).head(topK).copy()
        if nbrs.empty:
            st.warning("No neighbors found; try increasing Top-K or adjusting weights."); return

        st.session_state["H6_NEIGHBORS"]=nbrs[["seed","winner","seed_digits","winner_digits","similarity"]].copy()
        st.success(f"Locked {len(st.session_state['H6_NEIGHBORS'])} (seed,winner) pairs.")
        st.dataframe(st.session_state["H6_NEIGHBORS"].head(50), hide_index=True, use_container_width=True)

    st.subheader("Step 2 · Evaluate filters on locked neighbors (per-row seed)")
    filt_file = st.file_uploader("Filters CSV (id + expression required)", type=["csv"], key="h6_filters")
    limit_ids = st.checkbox("Limit to pasted Filter IDs", value=False, key="h6_limit_ids")
    ids_txt   = st.text_area("Paste IDs (comma/space/newline separated)", "", height=100, key="h6_ids")
    semantics = st.radio("Elimination semantics", ["Expression True ⇒ ELIMINATE", "Expression True ⇒ KEEP"], index=0, key="h6_sem")

    if st.button("Run Historical Safety (weighted + unweighted)", key="h6_s2"):
        if "H6_NEIGHBORS" not in st.session_state:
        ...
        for _, r in dff.iterrows():
            norm = normalize_expr(str(r["expression"]))
            try:
                code = compile(norm or "False", "<hist_expr>", "eval")
            except SyntaxError:
                code = compile("False", "<hist_expr>", "eval")
            comp.append({"id":str(r["id"]), "name":str(r["name"]), "code":code})
        comp=pd.DataFrame(comp)

        nbrs=st.session_state["H6_NEIGHBORS"]
        true_means_eliminate = (semantics.startswith("Expression True"))

        if prior.startswith("Laplace"):
            a0=b0=1.0
        else:
            a0=b0=0.0

        rows=[]
        total_weight_all_filters = 0.0
        weighted_safety_sum = 0.0
        unweighted_safety_sum = 0.0
        n_filters_with_valid = 0

        for _, fr in comp.iterrows():
            fid,fname,code=fr["id"],fr["name"],fr["code"]
            valid=err=elim=0
            elim_w = 0.0; tot_w = 0.0
            for _, n in nbrs.iterrows():
                W = float(n["similarity"])
                seed_str = "".join(str(d) for d in n["seed_digits"])
                base_env = make_base_env(seed_str, "", "", "",
                                         st.session_state.get("hot_digits", []),
                                         st.session_state.get("cold_digits", []),
                                         st.session_state.get("due_digits", []))
            ...
            if valid>0:
                n_filters_with_valid += 1
                aggr_unw = elim/valid; safe_unw = 1.0 - aggr_unw
                aggr_w_raw = (elim_w/tot_w) if tot_w>0 else 0.0
                aggr_w = _beta_shrink(elim_w, tot_w, a0, b0) if (a0+b0)>0 else aggr_w_raw
                safe_w = 1.0 - aggr_w
                weighted_safety_sum += safe_w * tot_w
                total_weight_all_filters += tot_w
                unweighted_safety_sum += safe_unw
            else:
                safe_unw = 0.0; safe_w = 0.0

            rows.append({
                "id":fid,"name":fname,"valid_cases":valid,"error_cases":err,
                "weighted_total": round(tot_w,4),"weighted_eliminated": round(elim_w,4),
                "historical_safety_unweighted_%": round(safe_unw*100,2),
                "historical_safety_weighted_%": round(safe_w*100,2)
            })

        out=pd.DataFrame(rows).sort_values(
            ["historical_safety_weighted_%","historical_safety_unweighted_%","valid_cases"],
            ascending=[False,False,False]
        ).reset_index(drop=True)

        overall_unweighted = (unweighted_safety_sum / n_filters_with_valid)*100.0 if n_filters_with_valid>0 else 0.0
        overall_weighted   = (weighted_safety_sum / total_weight_all_filters)*100.0 if total_weight_all_filters>0 else 0.0

        st.success(f"Evaluated {len(out)} filters against {len(nbrs)} (seed,winner) pairs.")
        st.markdown(f"### Overall historical safety (unweighted): **{overall_unweighted:.2f}%**")
        st.markdown(f"### Overall historical safety (weighted by similarity): **{overall_weighted:.2f}%**")
        st.caption("Unweighted = mean of per-filter safety. Weighted = similarity-weighted across all eliminations/totals.")
