...
def HFR_render():
    ...
    with c1:
        hist_file = st.file_uploader("Upload seed/winner history CSV or TXT", type=["csv","txt"],
                                     help="Order is preserved as provided (chronological or reverse). If only a 'Result' column is present, seeds are auto-generated by shifting rows.")
    ...
    if seed_col is None and winner_col is None:
        if len(df_hist.columns) == 1:
            ...
            cols_lower_map = {"result": "Result"}
            winner_col = "Result"
        else:
            if "result" in cols_lower_map:
                winner_col = cols_lower_map["result"]
            elif "draw" in cols_lower_map:
                winner_col = cols_lower_map["draw"]

    if winner_col is None:
        st.error(f"History file must include a results column (e.g., 'Result'). Found: {list(df_hist.columns)}")
        return

    if seed_col is None:
        res_series = df_hist[winner_col].astype(str).tolist()
        n = len(res_series)
        if n < 2:
            st.error("History needs at least two rows to generate seeds from results.")
            return
        if reverse_hint:
            gen_seed = res_series[1:]
            gen_win = res_series[:-1]
        else:
            gen_seed = res_series[:-1]
            gen_win = res_series[1:]
        df_hist = pd.DataFrame({"seed": gen_seed, "winner": gen_win})
        seed_col, winner_col = "seed", "winner"
    else:
    ...
    for _, f in df_filters.iterrows():
        ...
        results.append({
            ...
            "similar_cases": len(nbrs),
            "valid_cases": valid,
            "error_cases": errors,
            "elimination_rate_pct": round(elim_rate * 100.0, 2),
            "historical_safety_pct": round(safety * 100.0, 2),
        })
    ...
    res = pd.DataFrame(results).sort_values(["historical_safety_pct","valid_cases","similar_cases"], ascending=[False, False, False]).reset_index(drop=True)
