...
# Delegation to base evaluator if present
def HFR_get_base_evaluator():
    candidates = ["evaluate_filter","evaluate_expression","apply_filter_expression",
...
def HFR_eval_with_base_or_fallback(base_fn, expr_text, combo_digits, seed_digits):
    ctx = HFR_build_combo_context(combo_digits, seed_digits)
    if base_fn is not None:
        # Try common signatures
        try: return bool(base_fn(expr_text, ctx))
        except Exception:
            try: return bool(base_fn(expr_text, combo_digits, seed_digits))
            except Exception:
                try: return bool(base_fn(ctx, expr_text))
                except Exception: pass
    # Fallback: safe eval
    try:
        compiled_expr = compile(expr_text, "<filter_expr>", "eval")
    except Exception as e:
        raise e
    return bool(eval(compiled_expr, {"__builtins__": {}}, {**ctx, **SAFE_BUILTINS}))

def HFR_render():
    import streamlit as st

    st.markdown("---")
    st.header("Historical Safety Recommender — Reliable (Additive)")

    c1, c2 = st.columns(2)
    with c1:
        hist_file = st.file_uploader("Upload history CSV/TXT (Result OK; seeds auto-shifted)", type=["csv","txt"])
    ...
    with st.expander("Similarity", expanded=False):
        ...
        K_init  = st.slider("Initial Top-K neighbors", 50, 500, 200, 10)
        thr_init = st.slider("Initial Minimum similarity", 0.0, 8.0, 3.0, 0.1)
        min_valid_target = st.slider("Target valid evals per filter", 50, 500, 150, 10)
        thr_step = st.slider("Similarity relax step", 0.1, 1.0, 0.2, 0.1)
    ...
    with c4:
        ...
        ids_from_csv = st.file_uploader("Or upload IDs CSV (with 'id' column)", type=["csv"], key="ids_csv")
    ...
    if win_col is None: st.error(f"History must include a results column (e.g., 'Result'). Found: {list(dfh.columns)}"); return
    ...
    dfh["p_spr"]  = dfh["seed_digits"].apply(HFR_spread)

    prof_now = {"sum":HFR_sum_of_digits(seed_now),"sum_category":HFR_sum_category(HFR_sum_of_digits(seed_now)),
                "even":HFR_parity_counts(seed_now)[0],"odd":HFR_parity_counts(seed_now)[1],
                "high":HFR_high_low_counts(seed_now)[0],"low":HFR_high_low_counts(seed_now)[1],
                "vtrac":HFR_vtrac_groups(seed_now),"spread":HFR_spread(seed_now)}

    dfh["similarity"] = [
        HFR_similarity_score(prof_now, {"sum":r.p_sum,"sum_category":r.p_sumcat,"even":r.p_even,"odd":r.p_odd,
                                        "high":r.p_high,"low":r.p_low,"vtrac":r.p_vtr,"spread":r.p_spr}, weights)
        for _, r in dfh.iterrows()
    ]

    # Adaptive neighbor selection to reach target valid evals
    thr = thr_init
    K = K_init
    for _ in range(20):
        nbrs = dfh[dfh["similarity"]>=thr].sort_values("similarity", ascending=False).head(K)
        if len(nbrs) >= min_valid_target:
            break
        # relax similarity threshold first, then increase K
        thr = max(0.0, thr - thr_step)
        if len(dfh[dfh["similarity"]>=thr]) < K:
            K = min(len(dfh), K + 50)
    nbrs = dfh[dfh["similarity"]>=thr].sort_values("similarity", ascending=False).head(K)
    if nbrs.empty:
        import streamlit as st
        st.warning("No neighbors — relax similarity or increase K.")
        return

    # Normalize weights for similarity-weighted safety
    sim_vals = nbrs["similarity"].astype(float).tolist()
    sim_min, sim_max = min(sim_vals), max(sim_vals)
    if sim_max > sim_min:
        nbrs["sim_weight"] = [(s - sim_min) / (sim_max - sim_min) for s in sim_vals]
    else:
        nbrs["sim_weight"] = [1.0 for _ in sim_vals]
    ...
    base_name, base_fn = HFR_get_base_evaluator()

    # Pre-collect neighbor contexts
    neighbor_data = list(zip(nbrs["seed_digits"].tolist(), nbrs["winner_digits"].tolist(), nbrs["sim_weight"].tolist()))
    ...
    for _, frow in dff.iterrows():
        ...
        for seed_d, combo_d, w in neighbor_data:
            try:
                out = HFR_eval_with_base_or_fallback(base_fn, expr_text, combo_d, seed_d)
                valid += 1
                w_valid += w
                if out: 
                    elim += 1
                ...
                err += 1  # errors excluded from denominator by default

        elim_rate = (elim / valid) if valid else 0.0
        safety = 1.0 - elim_rate if valid else 0.0
        w_elim_rate = (w_elim / w_valid) if w_valid else 0.0
        w_safety = 1.0 - w_elim_rate if w_valid else 0.0

        results.append({
            "id": fid, "name": fname,
            "valid_cases": valid, "error_cases": err,
            "historical_safety_%": round(safety*100.0, 2),
            "weighted_safety_%": round(w_safety*100.0, 2)
        })
    ...
    res = pd.DataFrame(results).sort_values(
        ["weighted_safety_%","historical_safety_%","valid_cases"],
        ascending=[False, False, False]
    ).reset_index(drop=True)

    import streamlit as st
    st.subheader("Recommended Filters — Historical Safety (weighted & unweighted %)")
    st.caption(f"Evaluator used: {'base → ' + base_name if base_fn else 'fallback (safe)'} | Neighbors: {len(nbrs)} | thr≈{thr:.2f}, K≈{K}")
...
try:
    ...
    with st.sidebar:
        ...
        _HFR_SHOW = st.checkbox("Show Historical Safety Recommender (Reliable)", value=False)
    if _HFR_SHOW:
        HFR_render()
except Exception:
    pass
