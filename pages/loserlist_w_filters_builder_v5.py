...
def compute_numeric_context(last13: List[str], last20_opt: List[str]) -> Dict:
    ...
    seed_digits, prev_digits = rows[0], rows[1]
    ...
    seed_sum = sum(int(x) for x in seed_digits); prev_sum = sum(int(x) for x in prev_digits)
    ...
    return dict(
        ...
        order_prev=order_prev, order_curr=order_curr, rank_prev=rank_prev, rank_curr=rank_curr,
        ...
        seed_plus1=seed_plus1, seed_minus1=seed_minus1, union_digits=union_digits,
        carry_top2=carry_top2, union2=union2,
        prev_core_currentmap_digits=prev_core_currentmap_digits,
        core_size_flags=core_size_flags, trans_FI=trans_FI, trans_GI=trans_GI,
        seed_sum=seed_sum, prev_sum=prev_sum, loser_ranking=loser_ranking
    )
...
def resolve_expression(expr: str, ctx: Dict) -> str:
    ...
    # Enumerated short tokens: s1..s5 (seed), p1..p5 (seed+1), c1.. (carry top2), u1.. (union2)
    def _replace_enum(prefix: str, values: List[str]) -> None:
        nonlocal x
        for i, v in enumerate(values, start=1):
            x = re.sub(rf"\\b{prefix}{i}\\b", str(int(v)), x)

    _replace_enum('s', ctx.get('seed_digits', []))
    _replace_enum('p', ctx.get('seed_plus1', []))
    _replace_enum('c', ctx.get('carry_top2', []))
    _replace_enum('u', ctx.get('union2', []))
...
st.title("⚡ Loser List → Tester-ready Export (Hardened 15-col Output + s1/p1/c1/u1 expansion)")
...
if compute:
    try:
        ...
        last20_opt = parse_winners_text(st.session_state.get("winners20_text",""), pad4=st.session_state.get("pad4", True)) if st.session_state.get("winners20_text") else []
        if len(last13) < 13:
            st.error("Please provide at least 13 winners (MR→Oldest).")
        else:
            st.session_state["ctx"] = compute_numeric_context(last13, last20_opt)
...
if "ctx" in st.session_state:
    ...
    if st.button("Build 15-column CSV", type="primary"):
        try:
            ...
            csv_text = to_csv_quoted_all(df15)
            st.code(csv_text, language="csv")
            st.download_button("Download 15-column CSV", data=csv_text.encode("utf-8"),
                               file_name="filters_15col.csv", mime="text/csv", key="dl15v5")
        except Exception as e:
            st.error(str(e))
else:
