...
# Accepts single-column history (Result) and auto-generates seeds by shifting, respecting reverse order.
# Allows limiting evaluation to pasted/CSV Filter IDs and optionally uses 'enabled' column.
...
def HFR_render():
    ...
    with c1:
        hist_file = st.file_uploader("Upload seed/winner history CSV or TXT", type=["csv","txt"],
                                     help="Order is preserved as provided (chronological *or* reverse). If only a single 'Result' column is present, seeds will be auto-generated by shifting rows.")
    ...
    with id_col1:
        limit_to_ids = st.checkbox("Limit to pasted Filter IDs", value=False,
                                   help="If enabled, only the filters whose IDs you paste below will be evaluated.")
    with id_col2:
    ...
    pasted_ids = st.text_area("Paste Filter IDs (comma/space/newline separated)", value="", height=100,
                              help="Example: F001, F007 F123\\nF200")
    ids_from_csv = st.file_uploader("Optional: Upload a CSV containing a column named 'id' with the Filter IDs to include", type=["csv"], key="ids_csv")
