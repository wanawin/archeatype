...
def greedy_plan(candidates: pd.DataFrame, pool: List[str], base_env: Dict,
    ...
    for step in range(int(max_steps)):
        ...
        if stop_when_remaining_at_most is not None and len(remaining) <= stop_when_remaining_at_most:
            break
        scored = []
...
export_scored = st.sidebar.checkbox("Export scored filters table", value=False)

# ---------------- Main UI ----------------
st.subheader("Hot / Cold / Due digits (manual entry)")
...
if not user_provided_hcd and auto_hcd and winners_list:
    hist_digits = [digits_of(x) for x in winners_list][-auto_hcd_window:]
    ...
    if cnt:
        ...
        st.caption(f"(Auto) Hot/Cold/Due from last {auto_hcd_window} winners → hot={hot_digits} • cold={cold_digits} • due={due_digits}")
else:
    st.caption(f"(Using manual H/C/D) hot={hot_digits} • cold={cold_digits} • due={due_digits}")
