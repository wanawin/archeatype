...
# - seed_digits now maps to Draw 1-back (the "seed" in your filters).
# - winner_digits exposes the Known winner (0-back) separately.
# - All prior fixes retained: hybrid mirror, seed_sum, H/C/D, vtracs, skip reporting.
...
# ── Environments ──────────────────────────────────────────────────────────────
def make_base_env(winner, prev_seed, prev_prev_seed, prev_prev_prev_seed,
                  hot_digits, cold_digits, due_digits) -> Dict:
    # winner -> 0-back (exposed as winner_digits)
    # seed    -> 1-back (this is what filters expect: seed_digits)
    winner_digits = digits_of(winner) if winner else []
    seed_digits_1 = digits_of(prev_seed) if prev_seed else []  # seed (1-back)

    env = {
        "winner_digits": winner_digits,        # 0-back
        "seed_digits": seed_digits_1,          # 1-back (the "seed" your filters use)
        "prev_seed_digits": seed_digits_1,     # keep identical for compatibility
    ...
    winner_value = as_int_from_digits(env["winner_digits"])
    seed_sum_val = sum(env["seed_digits"])  # used by seed-based filters
    ...
    env.update({
        ...
        "winner_root_sum": digital_root(winner_value),
        "seed_sum": seed_sum_val,
...
winner        = s1.text_input("Known winner (0-back)", value="")
...
def run_planner_and_cache():
    base_env = make_base_env(
        winner, prev_seed, prev_prev, prev_prev_prev,
    ...
    if winner and not out.empty:
        rows_keep = []
        for _, r in out.iterrows():
            row_full = filters_df.loc[filters_df["id"]==r["id"]].iloc[0]
            env_seed = combo_env(base_env, winner)  # check against 0-back
            try:
