...
if not pool: st.warning("No pool loaded yet.")
...
try:
    ...
    st.warning("Could not load filters CSV — continuing with empty filters.")
    filters_df_full = pd.DataFrame(columns=["id","expression"])
...
if "enabled" in filters_df_full.columns:
    filters_df_full = filters_df_full[filters_df_full["enabled"]==True]

applicable_ids=set(parse_list(ids_text))
if applicable_ids:
    filters_df=filters_df_full[filters_df_full["id"].astype(str).isin(applicable_ids)].copy()
else:
...
if st.button("▶ Run Planner"):
    ...
    for _,r in filters_df.iterrows():
        ...
        scored_rows.append({"id":r["id"],"name":r.get("name",""),"expression":r["expression"],
    ...
    large_df=scored_df[scored_df["elim_count_on_pool"]>=int(min_elims)]
    st.write(f"Large filters ≥{min_elims}: {len(large_df)}")
    if large_df.empty: kept_after=pool; plan=[]
    else:
        plan,kept_after=greedy_plan(large_df[["id","name","expression"]],pool,base_env,int(beam_width),int(max_steps))
    st.write(f"Kept combos after plan: {len(kept_after)} / {len(pool)}")
    if plan: st.dataframe(pd.DataFrame(plan))
    st.download_button("Download KEPT CSV", pd.DataFrame({"Result":kept_after}).to_csv(index=False), "kept_combos.csv")
    st.download_button("Download REMOVED CSV", pd.DataFrame({"Result":sorted(set(pool)-set(kept_after))}).to_csv(index=False), "removed_combos.csv")
