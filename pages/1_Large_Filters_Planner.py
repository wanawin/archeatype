...
def parse_list(text: str, to_int: bool=False) -> List[str | int]:
    raw = (
        text.replace("\t", ",")
            .replace("\n", ",")
            .replace(" ", ",")
...
# ---------- Hot/Cold/Due from history ----------
def hot_cold_due_from_history(winners: List[str], k: int = 30):
    """Derive hot/cold/due from most recent k draws in history if present."""
    if not winners:
        return [], [], []
    hist = winners[-k:] if len(winners) > k else winners
    flat = [d for w in hist for d in digits_of(w)]
    if not flat:
        return [], [], []
    cnt = Counter(flat)
    # hot = top ~6 by freq; cold = bottom ~4 by freq; due = not seen in last 2 draws
    most = cnt.most_common()
    topk = 6
    hot = sorted({d for d, c in most if c >= (most[topk-1][1] if len(most) >= topk else most[-1][1])})
    least = sorted(cnt.items(), key=lambda x: (x[1], x[0]))
    coldk = 4
    cold = sorted({d for d, c in least if c <= (least[coldk-1][1] if len(least) >= coldk else least[0][1])})
    last2 = set(d for w in winners[-2:] for d in digits_of(w))
    due = sorted([d for d in range(10) if d not in last2])
    return hot, cold, due
...
# ===============================
def load_pool_from_csv(data, col_hint: str | None) -> List[str]:
    df = pd.read_csv(data)
...
def parse_pool_text(text: str) -> List[str]:
    """Accept CSV text with a 'Result' column OR free-form 1-per-line / comma separated combos."""
    txt = text.strip()
    if not txt:
        return []
    # Try CSV with 'Result' header
    try:
        df = pd.read_csv(io.StringIO(txt))
        cols_lower = {c.lower(): c for c in df.columns}
        if "result" in cols_lower:
            return [str(x).strip() for x in df[cols_lower["result"]].dropna().astype(str)]
    except Exception:
        pass
    # Fallback: split on commas/space/newlines
    return [p for p in parse_list(txt, to_int=False)]

def load_winners(history_source) -> List[str]:
    try:
...
def eval_bool_expr(expr: str, env: Dict) -> bool:
    try:
        code = compile(str(expr), "<expr>", "eval")
    except Exception:
        return False
    try:
        return bool(eval(code, SAFE_GLOBALS, env))
    except Exception:
        return False

def eval_filter_on_pool(row: pd.Series, pool: List[str], base_env: Dict) -> Tuple[Set[str], int]:
    eliminated: Set[str] = set()
    app_expr = str(row.get("applicable_if", "True"))
    fil_expr = str(row["expression"])
    for c in pool:
        env = combo_env(base_env, c)
        # check applicability first (if errors => not applicable)
        app_ok = eval_bool_expr(app_expr, env)
        if not app_ok:
            continue
        # expression True => eliminate
        if eval_bool_expr(fil_expr, env):
            eliminated.add(c)
...
with st.sidebar:
    ...
    mode = st.radio(
        ...
        index=1,
        help="Reducer: larger eliminations (kept logic). Explorer: lower threshold and deeper search."
    )
...
seed = c1.text_input("Seed (prev draw)", value="", max_chars=10, help="Required — 5 digits.")
prev_seed = c2.text_input("Prev Seed (2-back)", value="", max_chars=10)
prev_prev = c3.text_input("Prev Prev Seed (3-back)", value="", max_chars=10)

if len(digits_of(seed)) != 5:
...
hot_manual  = parse_list(hot_txt, to_int=True)
cold_manual = parse_list(cold_txt, to_int=True)
due_manual  = parse_list(due_txt, to_int=True)

# ===============================
# Combo Pool
# ===============================
st.subheader("Combo Pool")
pool_text = st.text_area("Paste combos (CSV with 'Result' column OR 1-per-line / comma/space separated):", height=130)
...
if pool_text.strip():
    ...
    try:
        pool = load_pool_from_csv(pool_file, pool_col_hint if pool_col_hint.strip() else None)
...
# ===============================
# Winners History (optional)
# ===============================
st.subheader("Winners History (optional)")
history_default_path = "DC5_Midday_Full_Cleaned_Expanded.csv"
c7, c8 = st.columns([2,1])
history_path = c7.text_input(
    "Path to winners history CSV",
    value=history_default_path,
    help="Leave as default or override."
)
history_file_up = c8.file_uploader("Or upload history CSV", type=["csv"], key="hist_upload")

history_source = history_file_up if history_file_up is not None else (history_path if history_path.strip() else None)
winners_list = load_winners(history_source)

# If H/C/D not provided, derive from history (if any)
if not (hot_manual or cold_manual or due_manual) and winners_list:
    h_auto, c_auto, d_auto = hot_cold_due_from_history(winners_list, k=30)
    hot_digits, cold_digits, due_digits = h_auto, c_auto, d_auto
else:
    hot_digits, cold_digits, due_digits = hot_manual, cold_manual, due_manual

# ===============================
# Filters section (THIS IS THE PART YOU ASKED FOR)
# ===============================
st.subheader("Filters")
ids_text = st.text_area(
    "Paste applicable Filter IDs (optional, comma/space/newline). If provided, they will be looked up in the selected Filters CSV.",
    height=80,
    help="Example: 1F475, 1F476, NO201F... etc."
)
...
filters_source = filters_upload if filters_upload is not None else filters_csv_path_default
...
# Subset by pasted IDs if any
applicable_ids = set([str(x) for x in parse_list(ids_text, to_int=False)])
if applicable_ids:
    id_str = filters_df_full["id"].astype(str)
    filters_df = filters_df_full[id_str.isin(applicable_ids)].copy()
    if filters_df.empty:
        st.error("None of the pasted IDs matched the selected Filters CSV.")
        st.stop()
else:
...
# ===============================
# Build base environment (uses H/C/D)
# ===============================
...
# RUN BUTTON
# ===============================
run = st.button("▶️ Run Planner", type="primary")

if not run:
    st.stop()

# ===============================
# Evaluate filters on the current pool
# ===============================
st.subheader(f"Running {mode} with {len(filters_df)} filters and {len(pool)} combos…")
scored_rows = []
...
st.dataframe(large_df[["id", "name", "elim_count_on_pool", "elim_pct_on_pool"]], use_container_width=True, hide_index=True)
...
cA, cB = st.columns(2)
cA.download_button("⬇️ Download KEPT combos (CSV)", kept_df.to_csv(index=False), file_name="kept_combos.csv", mime="text/csv")
cA.download_button("⬇️ Download KEPT combos (TXT)", "\n".join(kept_after), file_name="kept_combos.txt", mime="text/plain")
cB.download_button("⬇️ Download REMOVED combos (CSV)", removed_df.to_csv(index=False), file_name="removed_combos.csv", mime="text/csv")
cB.download_button("⬇️ Download REMOVED combos (TXT)", "\n".join(removed), file_name="removed_combos.txt", mime="text/plain")
