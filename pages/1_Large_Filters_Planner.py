...
# Page config
# -------------------------------
st.set_page_config(page_title="Archetype Helper — Large Filters, Triggers & Plans",
                   layout="wide")
...
def parse_list(text: str, to_int: bool=False) -> List[str]:
    """Split on commas, spaces, newlines, or semicolons."""
...
def load_winners_csv(path: str | None, uploaded) -> List[str]:
    try:
...
def load_filters_csv(source) -> pd.DataFrame:
    # source may be a path str/Path or an uploaded file-like
...
def eval_filter_on_pool(row: pd.Series, pool: List[str], base_env: Dict) -> Tuple[set[str], int]:
    ...
    for c in pool:
        ...
        try:
            ...
            # treat as non-elimination on error for this combo
            pass
...
with st.sidebar:
    ...
    mode = st.radio(
        ...
        index=1,
        help="Reducer = larger cuts (kept logic), Explorer = more candidates (lower threshold)."
    )
...
# -------------------------------
# Inputs (in a form) + RUN button
# -------------------------------
with st.form("planner_inputs"):
    st.subheader("Seed context")
    c1, c2, c3 = st.columns(3)
    seed      = c1.text_input("Seed (5 digits, required)", value="")
    ...
    hot_txt  = c4.text_input("Hot digits (comma/space/newline)", value="")
    cold_txt = c5.text_input("Cold digits (comma/space/newline)", value="")
    due_txt  = c6.text_input("Due digits (comma/space/newline)", value="")
    ...
    pool_file = st.file_uploader("Or upload pool CSV (needs a 'Result' column or set a column name)", type=["csv"])
    pool_col_hint = st.text_input("Pool column name (optional hint)", value="Result")

    st.subheader("Winners History (optional)")
    history_default = "DC5_Midday_Full_Cleaned_Expanded.csv"
    history_path = st.text_input("Path to winners history CSV", value=history_default)
    history_upload = st.file_uploader("…or upload winners history CSV", type=["csv"], key="hist_up")

    st.subheader("Filters")
    ids_text = st.text_area(
        "Paste applicable Filter IDs (optional, comma/space/newline). "
        "They will be matched against the active Filters CSV.",
        height=90,
    )
    ...
    run = st.form_submit_button("▶ Run Planner", use_container_width=True)

# Stop until user clicks RUN
if not run:
    st.info("Fill in inputs, then click **Run Planner**.")
    st.stop()
...
if pool_text.strip():
    ...
    st.error("Paste combos or upload a pool CSV to continue.")
    st.stop()
...
if not winners_list:
    st.warning("History CSV not found or empty. Proceeding without history-driven extras.")

# -------------------------------
# Load filters CSV (default or uploaded)
# -------------------------------
filters_default_path = "lottery_filters_batch_10.csv"
filters_source = filters_upload if filters_upload is not None else filters_default_path
...
# Apply pasted IDs (subset) if provided
...
if applicable_ids:
    ...
    if filters_df.empty:
        st.error("None of the pasted IDs matched rows in the Filters CSV.")
        st.stop()
else:
...
cA, cB = st.columns(2)
cA.download_button("Download KEPT combos (CSV)", kept_df.to_csv(index=False),
                   file_name="kept_combos.csv", mime="text/csv")
cA.download_button("Download KEPT combos (TXT)", "\n".join(kept_after),
                   file_name="kept_combos.txt", mime="text/plain")
cB.download_button("Download REMOVED combos (CSV)", removed_df.to_csv(index=False),
                   file_name="removed_combos.csv", mime="text/csv")
cB.download_button("Download REMOVED combos (TXT)", "\n".join(removed),
                   file_name="removed_combos.txt", mime="text/plain")
