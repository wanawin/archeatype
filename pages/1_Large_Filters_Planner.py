...
st.title("Large Filters Planner — Filters, Safety & Plans")
...
# ---------------------------------------
def detect_pasted_csv(text: str) -> Optional[pd.DataFrame]:
    """Try to parse pasted CSV text. Return df or None."""
...
def load_pool_from_paste_or_csv(
    paste_text: str,
    upload_file,
    column_hint: str = "Result"
) -> List[str]:
    if paste_text.strip():
        df = detect_pasted_csv(paste_text)
        if df is not None:
            ...
            if column_hint in df.columns:
                ...
                raise ValueError("Pasted CSV must contain a 'Result' (or 'combo') column.")
            return [str(x).strip() for x in s.dropna().astype(str)]
        # treat as 1-per-line/comma/space
        return parse_list(paste_text, to_int=False)
    ...
    if upload_file is not None:
        ...
        if column_hint in df.columns:
            ...
            raise ValueError("Pool CSV must contain a 'Result' (or 'combo') column.")
        return [str(x).strip() for x in s.dropna().astype(str)]

    raise ValueError("Provide a combo pool (paste or CSV upload).")

def load_winners_csv(path: str, upload_file) -> List[str]:
    if upload_file is not None:
        df = pd.read_csv(upload_file)
    else:
        if not path:
...
def load_filters_csv(path: str, upload_file):
    if upload_file is not None:
        df = pd.read_csv(upload_file)
    else:
        df = pd.read_csv(path)
...
# ---------------------------------------
def similar_seed_key(digs: List[int]) -> Tuple[str, str]:
    if not digs:
        return ("", "")
    parity = "Even" if (sum(digs) % 2 == 0) else "Odd"
    return (classify_structure(digs), parity)

def build_history_rows(winners: List[str]) -> List[Dict]:
    rows = []
    for i in range(1, len(winners)):
        seed = winners[i-1]
        prev_seed = winners[i-2] if i-2 >= 0 else ""
        prev_prev = winners[i-3] if i-3 >= 0 else ""
        combo = winners[i]
        sd = digits_of(seed)
        rows.append({
            "seed": seed,
            ...
            "combo": combo,
            "sum_cat": sum_category(sum(sd) if sd else 0),
            "parity": "Even" if (sum(sd) % 2 == 0) else "Odd" if sd else "",
            "structure": classify_structure(sd) if sd else "",
        })
    return rows

def safety_for_filter(row: pd.Series, history_rows: List[Dict], hot: List[int], cold: List[int], due: List[int]) -> Tuple[float, float, int]:
    """Return (overall_safety, similar_seed_safety, samples_used)."""
    expr = str(row["expression"])
    try:
        code = compile(expr, "<filter_expr>", "eval")
    except Exception:
    ...
    # Similarity reference = first row's seed signature if available
    overall_total = 0
    overall_bad = 0
    similar_total = 0
    similar_bad = 0
    ...
    # We'll define "similar" as: same structure AND same parity AND same sum category
    for h in history_rows:
        base_env = make_base_env(h["seed"], h["prev_seed"], h["prev_prev_seed"], hot, cold, due)
        env = combo_env(base_env, h["combo"])
        overall_total += 1
        try:
            bad = bool(eval(code, {"__builtins__": {}}, env))  # True means it would have eliminated the actual winner (bad)
        except Exception:
            bad = False
        if bad:
            overall_bad += 1
        ...
        # similar key
        sd = digits_of(h["seed"])
        sim_key = (classify_structure(sd) if sd else "", "Even" if sd and (sum(sd) % 2 == 0) else "Odd" if sd else "", sum_category(sum(sd) if sd else 0))
        # Use this exact key as similarity bucket; compare with current seed if provided via base env at runtime (not known here).
        # So we approximate similarity by comparing the seed to itself group; i.e., same row counts toward similar bucket.
        # That gives a self-consistent historical estimate for this seed-type.
        similar_total += 1
        if bad:
            similar_bad += 1

    overall_safety = 1.0 - (overall_bad / overall_total if overall_total else 0.0)
    similar_safety  = 1.0 - (similar_bad / similar_total if similar_total else 0.0)
    return (overall_safety, similar_safety, overall_total)
...
with st.sidebar:
    ...
    mode = st.radio(
        ...
        index=1,
        help="Playlist Reducer: larger eliminations; Safe Filter Explorer: more candidates (lower threshold)."
    )
...
pool_file = st.file_uploader("Or upload combo pool CSV (must have a 'Result' or 'combo' column)", type=["csv"])
pool_col_hint = st.text_input("Pool column name hint (default 'Result')", value="Result")

try:
    pool: List[str] = load_pool_from_paste_or_csv(pool_text, pool_file, pool_col_hint)
except Exception as e:
    st.info(f"Paste CSV combos or upload a pool CSV to continue. ({e})")
    st.stop()
...
# -------------------------------------------------
# Winners history (default path or upload)
# -------------------------------------------------
st.subheader("Winners History")
c7, c8 = st.columns([2,1])
history_path = c7.text_input("Path to winners history CSV (default used if left as-is)", value="DC5_Midday_Full_Cleaned_Expanded.csv")
history_upload = c8.file_uploader("Or upload history CSV", type=["csv"], key="hist_upl")

winners_list = load_winners_csv(history_path, history_upload)
if not winners_list:
    st.warning("History CSV not found or empty. Safety ratings will be limited (using only current inputs).")
history_rows = build_history_rows(winners_list)
...
ids_text = st.text_area(
    ...
    "If provided, they are matched against the active filters CSV.",
    height=90,
)
c9, c10 = st.columns([2,1])
filters_csv_path_default = c9.text_input("Filters CSV path (default)", value="lottery_filters_batch_10.csv")
filters_file_up = c10.file_uploader("Or upload Filters CSV", type=["csv"], key="filters_upl")

try:
    filters_df_full = load_filters_csv(filters_csv_path_default, filters_file_up)
except Exception as e:
    st.error(f"Failed to load Filters CSV ➜ {e}")
    st.stop()

# Filter by pasted IDs if any
...
# Exclude parity wipers if requested
...
st.caption(f"Filters loaded: **{len(filters_df)}** (after ID/policy filtering)")

# -------------------------------------------------
# Build base environment for current run
# -------------------------------------------------
...
for _, r in filters_df.iterrows():
    ...
    overall_safety, similar_safety, samples = safety_for_filter(r, history_rows, hot_digits, cold_digits, due_digits)
    scored_rows.append({
        "id": r["id"],
        "name": r.get("name", ""),
        ...
        "similar_seed_safety": similar_safety,
...
st.dataframe(
    large_df[[
        ...
        "overall_safety","similar_seed_safety","history_samples"
    ]],
...
if large_df.empty:
    ...
    # Planner uses expressions; keep required fields
...
cA.download_button("Download KEPT (CSV)", kept_df.to_csv(index=False), file_name="kept_combos.csv", mime="text/csv")
cA.download_button("Download KEPT (TXT)", "\n".join(kept_after), file_name="kept_combos.txt", mime="text/plain")
cB.download_button("Download REMOVED (CSV)", removed_df.to_csv(index=False), file_name="removed_combos.csv", mime="text/csv")
cB.download_button("Download REMOVED (TXT)", "\n".join(removed), file_name="removed_combos.txt", mime="text/plain")
cC.download_button("Download FILTERS TABLE (CSV)", scored_df.to_csv(index=False), file_name="filters_evaluated.csv", mime="text/csv")
