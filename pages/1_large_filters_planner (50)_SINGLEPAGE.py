...
# ===================== ADDITIVE: Historical Safety — Two-Step (Single Page) =====================
# This section does not modify any base functions; it only adds UI + logic.
import re, types, pandas as pd
from collections import Counter
from datetime import datetime

def _hsp_digits(s): return [int(ch) for ch in ''.join(ch for ch in str(s) if ch.isdigit())]
def _hsp_sum(d): return sum(d)
def _hsp_parity(d): 
    ev = sum(1 for x in d if x % 2 == 0); return ev, len(d)-ev
def _hsp_hilo(d):
    lo = sum(1 for x in d if x in {0,1,2,3,4}); hi = len(d)-lo; return hi, lo
_VTR = {0:1,5:1,1:2,6:2,2:3,7:3,3:4,8:4,4:5,9:5}
def _hsp_vtr(d): return [_VTR[x] for x in d]
def _hsp_spread(d): return max(d)-min(d) if d else 0
def _hsp_sumcat(t): 
    return ('Very Low' if t<=15 else 'Low' if t<=24 else 'Mid' if t<=33 else 'High')

def _hsp_sim_score(p_now, p_hist, w=None):
...
_SAFE = {'len':len,'sum':sum,'min':min,'max':max,'abs':abs,'any':any,'all':all,'sorted':sorted,'set':set,'tuple':tuple,'list':list,'range':range,'round':round}

def _hsp_get_base_eval():
    g = globals()
    for name in ["evaluate_filter","evaluate_expression","apply_filter_expression","passes_filter","filter_eval","filter_applies","eliminates_combo"]:
        fn = g.get(name)
        if isinstance(fn, types.FunctionType):
            return name, fn
    return None, None

def _hsp_ctx(combo, seed):
    d0,d1,d2,d3,d4 = (combo+[None]*5)[:5]
    ev,od = _hsp_parity(combo); hi,lo = _hsp_hilo(combo)
    return {
        "combo_digits":combo,"seed_digits":seed,"d0":d0,"d1":d1,"d2":d2,"d3":d3,"d4":d4,
        "combo_sum":_hsp_sum(combo),"combo_spread":_hsp_spread(combo),
        "even_count":ev,"odd_count":od,"high_count":hi,"low_count":lo,"vtrac":_hsp_vtr(combo),
        "digits":combo,"draw_digits":combo,"result":combo,"winner":combo,"win_digits":combo,"current_digits":combo,
        "prev_digits":seed,"seed":seed
    }

def _hsp_eval_bool(expr, combo, seed, base_fn):
    if base_fn is not None:
        for sig in [(expr, _hsp_ctx(combo, seed)), (expr, combo, seed), (_hsp_ctx(combo, seed), expr)]:
            try:
                val = base_fn(*sig)
                ...
                pass
    try:
        compiled = compile(expr, "<filter_expr>", "eval")
        val = eval(compiled, {"__builtins__": {}}, {**_hsp_ctx(combo, seed), **_SAFE})
        if isinstance(val, bool): return True, val, None
        return False, False, "non_boolean"
    except Exception:
        return False, False, "exception"

def render_historical_two_step():
    import streamlit as st
    with st.sidebar:
        st.markdown("---")
        show = st.checkbox("Show Historical Safety — Two-Step", value=False, help="Adds historical recommendations to THIS page")
    if not show: 
        return

    st.markdown("## Historical Safety — Two-Step (on this page)")

    c1,c2 = st.columns(2)
    with c1:
        hist_file = st.file_uploader("History CSV/TXT", type=["csv","txt"], key="hsp_hist")
        reverse = st.checkbox("History is reverse chronological (newest first)", value=True, key="hsp_rev")
    with c2:
        seed_txt = st.text_input("Current seed (5 digits)", "27500", key="hsp_seed")

    seed_now = _hsp_digits(seed_txt)
    if len(seed_now)!=5:
        st.error("Seed must be 5 digits."); return

    with st.expander("Similarity settings", expanded=False):
        w = {
            "sum": st.slider("Weight: Sum",0.0,3.0,1.0,0.1,key="hsp_wsum"),
            "sum_category": st.slider("Weight: Sum Category",0.0,3.0,1.5,0.1,key="hsp_wsumcat"),
            "even": st.slider("Weight: Even",0.0,3.0,0.6,0.1,key="hsp_wev"),
            "odd": st.slider("Weight: Odd",0.0,3.0,0.6,0.1,key="hsp_wod"),
            "high": st.slider("Weight: High",0.0,3.0,0.8,0.1,key="hsp_whi"),
            "low": st.slider("Weight: Low",0.0,3.0,0.8,0.1,key="hsp_wlo"),
            "spread": st.slider("Weight: Spread",0.0,3.0,0.8,0.1,key="hsp_wsp"),
            "vtrac": st.slider("Weight: V‑Trac",0.0,3.0,1.2,0.1,key="hsp_wvt"),
        }
        K = st.slider("Top‑K neighbors",10,500,200,5,key="hsp_k")
        thr = st.slider("Minimum similarity",0.0,8.0,3.0,0.1,key="hsp_thr")

    if st.button("Step 1 — Lock similar seeds", key="hsp_step1"):
        if not hist_file:
            st.error("Upload a history file to run."); return
        try:
        ...
        dfh["seed_digits"]   = dfh["seed"].apply(_hsp_digits)
        dfh["winner_digits"] = dfh["winner"].apply(_hsp_digits)
        dfh = dfh[dfh["seed_digits"].apply(len)==5]
        dfh = dfh[dfh["winner_digits"].apply(len)==5].reset_index(drop=True)

        dfh["p_sum"] = dfh["seed_digits"].apply(_hsp_sum)
        dfh["p_sumcat"] = dfh["p_sum"].apply(_hsp_sumcat)
        dfh["p_even"] = dfh["seed_digits"].apply(lambda d:_hsp_parity(d)[0])
        dfh["p_odd"]  = dfh["seed_digits"].apply(lambda d:_hsp_parity(d)[1])
        dfh["p_high"] = dfh["seed_digits"].apply(lambda d:_hsp_hilo(d)[0])
        dfh["p_low"]  = dfh["seed_digits"].apply(lambda d:_hsp_hilo(d)[1])
        dfh["p_vtr"]  = dfh["seed_digits"].apply(_hsp_vtr)
        dfh["p_spr"]  = dfh["seed_digits"].apply(_hsp_spread)

        prof_now = {"sum":_hsp_sum(seed_now),"sum_category":_hsp_sumcat(_hsp_sum(seed_now)),
                    "even":_hsp_parity(seed_now)[0],"odd":_hsp_parity(seed_now)[1],
                    "high":_hsp_hilo(seed_now)[0],"low":_hsp_hilo(seed_now)[1],
                    "vtrac":_hsp_vtr(seed_now),"spread":_hsp_spread(seed_now)}

        dfh["similarity"] = [
            _hsp_sim_score(prof_now, {"sum":r.p_sum,"sum_category":r.p_sumcat,"even":r.p_even,"odd":r.p_odd,
                                      "high":r.p_high,"low":r.p_low,"vtrac":r.p_vtr,"spread":r.p_spr}, w)
            for _, r in dfh.iterrows()
        ]
        nbrs = dfh[dfh["similarity"]>=thr].sort_values("similarity",ascending=False).head(K)
        if nbrs.empty:
            st.warning("No neighbors — relax similarity or increase Top‑K."); return
        st.success(f"Locked {len(nbrs)} similar seeds.")
        st.session_state["_HSP_NEIGHBORS"] = nbrs[["seed","winner","seed_digits","winner_digits","similarity"]].copy()
        st.dataframe(st.session_state["_HSP_NEIGHBORS"].head(50), hide_index=True, use_container_width=True)

        wins_df = st.session_state["_HSP_NEIGHBORS"][["winner"]].rename(columns={"winner":"winner_draw"})
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        st.download_button("Download winners from similar seeds (CSV)", wins_df.to_csv(index=False).encode("utf-8"),
                           file_name=f"similar_seed_winners_{ts}.csv", mime="text/csv")

    st.markdown("### Step 2 — Evaluate filters on locked winners")
    filt_file = st.file_uploader("Filters CSV (id + expression required)", type=["csv"], key="hsp_filters")
    use_enabled = st.checkbox("Use only enabled==True (if present)", value=False, key="hsp_en")
    limit_ids = st.checkbox("Limit to pasted Filter IDs", value=False, key="hsp_lim")
    ids_text = st.text_area("Paste IDs (comma/space/newline separated)", "", height=100, key="hsp_ids")

    run2 = st.button("Evaluate on locked winners", key="hsp_step2")

    if run2:
        if "_HSP_NEIGHBORS" not in st.session_state:
        ...
        if limit_ids and ids_text.strip():
            import re as _re
            tokens = _re.split(r"[,\s]+", ids_text.strip())
            keeps = {t.strip() for t in tokens if t.strip()}
            dff = dff[dff["id"].astype(str).isin(keeps)].copy()

        base_name, base_fn = _hsp_get_base_eval()

        nbrs = st.session_state["_HSP_NEIGHBORS"]
        results = []
        for _, row in dff.iterrows():
            fid, fname, expr = str(row["id"]), str(row["name"]), str(row["expression"]).strip()
            if not expr: continue
            valid = err_nb = err_ex = elim = 0
            for _, n in nbrs.iterrows():
                seed_d, combo_d = n["seed_digits"], n["winner_digits"]
                is_bool, val, err = _hsp_eval_bool(expr, combo_d, seed_d, base_fn)
                if err is None:
                    valid += 1
            ...
            results.append({
                "id":fid,"name":fname,"valid_cases":valid,
                "error_cases_nonbool":err_nb,"error_cases_exceptions":err_ex,
                "historical_safety_%":round(safety*100,2),
                "evaluator": ("base" if base_fn else "fallback")
            })
        if not results:
        ...
        st.success(f"Evaluated {len(out)} filters vs {len(nbrs)} winners.")
...
# Hook: render section on same page without altering base UI above
try:
    import streamlit as st
    render_historical_two_step()
except Exception:
    pass
