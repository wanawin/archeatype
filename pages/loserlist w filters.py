...
def to_three_cols(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if {"name","description","expression"}.issubset(cols) and "id" not in cols:
        ...
        # FIX: robust header-row stripping
        out = out[out["name"].astype(str).str.strip().str.strip('"').str.lower() != "name"]
    ...
    if needed.issubset(cols):
        ...
        # FIX: robust header-row stripping
        out3 = out3[out3["name"].astype(str).str.strip().str.strip('"').str.lower() != "name"]
