...
def _ring_digits(prev_core_letters: Set[str], digit_current_letters: Dict[str,str]) -> List[str]:
    if not prev_core_letters: return []
    neigh = set()
    for L in prev_core_letters:
        if L in LETTERS:
            i = LETTERS.index(L)
            if i-1 >= 0: neigh.add(LETTERS[i-1])
            if i+1 < 10: neigh.add(LETTERS[i+1])
...
def _read_csv_loose(text: str) -> pd.DataFrame:
    ...
    for sep in [",",";","\t","|"]:
        try:
            df = pd.read_csv(io.StringIO(t), sep=sep, engine="python",
...
def _to_three_cols(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not expr_col: raise ValueError("No 'expression' column found.")
    ...
    # drop accidental header rows like "name,description,expression"
    return out[out["name"].str.lower()!="name"].reset_index(drop=True)
...
def _replace_sets(expr: str,
    ...
    def fmt(xs): return "[" + ",".join(f"'{d}'" for d in sorted(xs)) + "]"
    out = expr
    ...
    def _sub(m):
        ...
        return f"{dvar} in [{','.join(f\"'{x}'\" for x in digs)}]"
    out = pat.sub(_sub, out)

    def letter_bool(txt, varname, s):
...
def digits_only_df(in_csv_text: str,
    ...
    for _, r in df_in.iterrows():
        ...
        expr = _replace_sets(expr, byL, prev_core_letters, cooled_digits, new_core_digits, loser_7_9, ring)
...
def to_tester_schema(df3: pd.DataFrame) -> pd.DataFrame:
    # Map 3-col → 5-col (+ Unnamed:5..14 like your tester export)
    out = pd.DataFrame({
        "id": df3["name"],
...
def neighbors(L: str, span:int=1) -> List[str]:
    i = LETTERS.index(L); lo,hi = max(0,i-span), min(9,i+span)
...
def parse_winners_text(txt: str, pad4: bool=False) -> List[str]:
    ...
    for t in toks:
        ...
        if len(t)!=5: raise ValueError("Each item must be 5 digits")
        out.append(t)
    return out

def loser_list(last13: List[str]) -> Tuple[List[str], Dict]:
    if len(last13)<13: raise ValueError("Need 13 winners (MR→Oldest).")
    ...
    core_letters = sorted({d2L_prev[d] for d in most_recent}, key=lambda L: LETTERS.index(L))
    ...
    tiers = {d: (3 if d2L_curr[d] in core_letters else
                 2 if (d2L_curr[d] in U and d in due) else
                 1 if d2L_curr[d] in U else 0) for d in DIGITS}
...
if compute:
    try:
        ...
        st.subheader("Loser list (Least → Most Likely)"); st.code(" ".join(ranking))
...
if "info" in st.session_state:
    ...
    with st.form("csv_form", clear_on_submit=False):
        src_choice = st.radio("Choose source", ["Paste MEGA CSV (3-col or 5-col)"], index=0)
        st.text_area("Paste CSV", key="mega_csv", height=180, value=st.session_state.get("mega_csv",""))
