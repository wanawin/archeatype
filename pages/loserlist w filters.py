...
def _replace_sets(expr: str,
    ...
    def fmt(xs): return "[" + ",".join("'" + d + "'" for d in sorted(xs)) + "]"
    out = expr

    # Replace simple symbolic sets with literal digit lists
...
def digits_only_df(in_csv_text: str,
    ...
    for _, r in df_in.iterrows():
        ...
        expr = _replace_sets(expr, byL, prev_core_letters,
...
def to_tester_schema(df3: pd.DataFrame) -> pd.DataFrame:
    # Map 3-col → 5-col (plus Unnamed:5..14 to match your tester CSVs)
    out = pd.DataFrame({
        "id":            df3["name"],
...
if compute:
    try:
        ...
        with colA:
            ...
            st.dataframe(pd.DataFrame({"digit": DIGITS,
