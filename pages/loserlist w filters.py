...
# loserlist_w_filters_builder.py (v2)
# Fix: add explicit "Build Tester CSV" button; persist ctx in session_state.
import io, re
...
# CSV Builder is always available once ctx exists
if "ctx" in st.session_state:
    ctx = st.session_state["ctx"]
    st.markdown("---")
    st.subheader("CSV → Tester Export")
    mega_csv = st.text_area("Paste filters CSV (3-col or 5-col)", height=220, key="mega_csv", help="Press the button below to build.")
    if st.button("Build Tester CSV", type="primary"):
        try:
            tester_df = build_tester_csv_from_paste(mega_csv or "", ctx)
