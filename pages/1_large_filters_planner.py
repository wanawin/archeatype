# 1_large_filters_planner_variants.py
# Full Streamlit app — same planner/UI as before, but with tester-style
# spelling-variation handling for filter expressions (ONLY this part changed).
...
# Page
# ------------------------------------------------------------
st.set_page_config(page_title="Large Filters Planner — FULL v6 (variants enabled)", layout="wide")
st.title("Large Filters Planner — FULL v6 (variants enabled)")
...
mirror = MIRROR  # some CSVs use lowercase
...
# ------------------------------------------------------------
# Filter spelling-variation handling (ONLY change)
# ------------------------------------------------------------
...
# word-boundary replace respecting identifiers
...
# canonicalization layer: just enough to match your tester
_VARIATION_MAP: Dict[str, str] = {
    # hot/cold/due variants
    ...
    "lastTwo": "last_two_digits(combo_digits)", "lasttwo": "last_two_digits(combo_digits)",
    ...
    # bare "sum" → digit sum unless used as function call; we map token here
...
def normalize_expr(expr: str) -> str:
    """
    Tester-style, minimal: ASCII sanitize then replace only known variants.
    No heavy parsing; just precise word-boundary rewrites to canonical names.
    """
    if not expr: return ""
    x = _ascii(expr)
    # quick camel→snake for a few known tokens (defensive)
    ...
    # apply controlled variant mappings (word boundaries)
    x = _wb_replace(x, _VARIATION_MAP)
    return x

def _clean_expr(s: str) -> str:
    s = str(s or "").strip().strip('"').strip("'")
    # typographical mistakes we’ve seen
    s = s.replace("!==", "!=")
    return normalize_expr(s)

# ------------------------------------------------------------
# CSV loaders for pool & history
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# FILTER LOADING — compile once (tester behavior), with normalization above
# ------------------------------------------------------------
@st.cache_data(show_spinner=False)
...
def normalize_filters_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # enabled default True if column missing
    ...
    for _, r in out.iterrows():
        ...
        try:
            ...
            # keep row disabled so UI still shows it's present
            rr["enabled"] = False
...
# ------------------------------------------------------------
# ENV BUILDERS (seed/combo) — H/C/D + mirror/vtrac wired to UI values
# ------------------------------------------------------------
...
def combo_env(base_env: Dict, combo: str) -> Dict:
    ...
    env.update({
        "combo": combo,
        "combo_digits": cd,  # keep original order
        "combo_set": cset,
        "combo_sum": sum(cd),
...
# ------------------------------------------------------------
# FILTER EVALUATION — tester behavior (compile once) + our variation layer
# ------------------------------------------------------------
def eval_applicable(row: pd.Series, base_env: Dict) -> bool:
    try:
        return bool(eval(row["applicable_code"], {"__builtins__": {}}, base_env))
    except Exception:
        # Treat as applicable to avoid false-negative skips
        return True
...
def eval_filter_on_pool(row: pd.Series, pool: List[str], base_env: Dict) -> Tuple[Set[str], int]:
    ...
    for c in pool:
        ...
        try:
            ...
            # per-combo error => treat as not eliminating that combo (same as tester)
            pass
    return eliminated, len(eliminated)

# ------------------------------------------------------------
# Greedy planner (unchanged behavior)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# UI — H/C/D, pool, winners, filters (unchanged)
# ------------------------------------------------------------
...
# Store for helpers that consult session state
...
# Auto H/C/D only if user left all boxes blank
...
seed      = st.text_input("Known winner (5 digits, optional)", value="")
prev_seed = st.text_input("Draw 1-back (optional)", value="")
prev_prev = st.text_input("Draw 2-back (optional)", value="")
...
# If user pasted just a list of IDs/names instead of a CSV, merge into fids_text
...
# Build env from UI values (H/C/D here are exactly the ones you typed)
...
# Optional: filter out parity wipers
...
# Evaluate all filters once to score
scores = []
pool_list = list(pool)
...
for _, r in candidates.iterrows():
    ...
    scores.append({
        ...
        "expression": r["expression"], "eliminated_now": cnt
    })
