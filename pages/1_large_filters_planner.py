...
# Consolidated: Sidebar (as requested) + tester-style filter engine + failure reporting
...
def normalize_filters_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, r in out.iterrows():
        rr = dict(r)
        # compile; if fail, keep row but mark compile_error and use a safe placeholder
        try:
...
def eval_filter_on_pool(row: pd.Series, pool: List[str], base_env: Dict,
    ...
    for c in pool:
        ...
        try:
            ...
            rec = runtime_errors_accum.setdefault(fid, {"id": fid, "name": fname, "error_type": "runtime", "error_count": 0, "first_error": ""})
            rec["error_count"] += 1
...
try:
    ...
    filters_df_full = pd.DataFrame(columns=[
        "id","name","expression","enabled","applicable_if",
        "compile_error_applicable","compile_error_expr"
    ])
...
if run:
    ...
    if len(filters_df) == 0:
        ...
        # Winner-preserving plan (exclude any filter that eliminates the known winner)
            ...
            if out.empty:
                ...
                if seed:
                    for _, r in out.iterrows():
                        ...
                        try:
                            knocks_winner = bool(eval(filters_df.loc[filters_df['id']==r['id']].iloc[0]["expr_code"],
                                                      {"__builtins__": {}}, env_seed))
                        except Exception:
                            knocks_winner = False
        ...
        # Playlist Reducer (simple greedy using beam/steps)
        ...
        if not out.empty:
            if mode == "Playlist Reducer":
                # Greedy beam: each step choose up to beam filters that remove the most remaining combos
                current_pool = set(kept_combos)
                candidates = out.copy()
                for step in range(greedy_steps):
                    if not candidates.empty and current_pool:
                        # recompute eliminations vs current pool
                        cand_rows = []
                        ...
                        for fid, nm, cnt, elimset in top:
                            ...
                            removed |= set(elimset)
                        # drop applied from candidates
                        candidates = candidates[~candidates["id"].astype(str).isin(applied_ids)]
                    else:
                        break
                kept_combos = sorted(list(current_pool))
            else:
                # Safe Filter Explorer: do not apply; just show current pool
                kept_combos = list(pool_list)

        # Show final pool and downloads
        ...
        # Skipped / Failed filters report
        st.subheader("Skipped / Failed filters")
        # also include compile errors captured earlier
        ...
        if skipped_records:
            ...
            st.download_button("Download skipped/failed filters (CSV)",
