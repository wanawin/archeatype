...
def resolve_expression(expr: str, ctx: Dict) -> str:
    ...
    # --- Minimal generator repair to avoid `name 'd' is not defined` ---
    # Add a missing `for d in combo_digits` when builder emits `sum(int(d) in X)` without a generator
    _MISSING_GEN = re.compile(r"\bsum\(\s*int\(d\)\s+(?:not\s+)?in\s*([^)]+)\)")
    x = _MISSING_GEN.sub(lambda m: f"sum(int(d) in {m.group(1)} for d in combo_digits)", x)

    # Also repair `sum(d in X)` (no int(d) and no generator)
    _MISSING_CAST_GEN = re.compile(r"\bsum\(\s*d\s+(?:not\s+)?in\s*([^)]+)\)")
    x = _MISSING_CAST_GEN.sub(lambda m: f"sum(int(d) in {m.group(1)} for d in combo_digits)", x)

    # And fix `sum(1 for in combo_digits if ...)` where `d` is missing after `for`
