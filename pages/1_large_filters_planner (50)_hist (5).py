...
# ===================== HFR ADDITIVE SECTION (SAFE & PERCENTAGES) =====================
# This section is appended to the base app without modifying or removing any existing logic.
# Features:
# - History uploader (CSV/TXT). If only 'Result' column present, auto-generate 'seed' by shifting (respecting order).
# - Similarity engine with adjustable weights, K, and min similarity.
# - Safe expression evaluation with a whitelist of builtins (len, sum, min, max, abs, any, all, sorted, set, tuple, list, range, round).
# - Variable aliases for legacy filters (result/digits/winner/etc.).
# - Percentage scoring (0–100%) with valid/error case counts.
# - Optionally limit evaluation to pasted Filter IDs or an IDs CSV.
# - Optional 'enabled==True' restriction if present in filters CSV.
# - Toggle to count expression errors as eliminations.
# NOTE: All names are prefixed with HFR_ and rendered via a sidebar toggle to avoid changing base UI.
...
def HFR_safe_eval_expression(expr: str, context: dict) -> bool:
    ...
    # Safe eval: builtins locked, only whitelisted helpers + context available
...
def HFR_render():
    ...
    with c1:
        hist_file = st.file_uploader(
            ...
            type=["csv","txt"],
            help="Order is preserved exactly (chronological or reverse). If only a 'Result' column is present, seeds are auto-generated by shifting rows."
        )
    ...
    st.markdown("#### Filter ID controls")
    id_col1, id_col2 = st.columns(2)
    with id_col1:
        limit_to_ids = st.checkbox("Limit to pasted Filter IDs", value=False)
    with id_col2:
        use_enabled_col = st.checkbox("Use only filters with enabled==True (if column exists)", value=False)

    pasted_ids = st.text_area("Paste Filter IDs (comma/space/newline separated)", value="", height=100)
    ids_from_csv = st.file_uploader("Optional: Upload CSV with an 'id' column of Filter IDs", type=["csv"], key="ids_csv")
    ...
    with st.expander("Similarity weights & neighborhood", expanded=False):
        ...
        k_neighbors = st.slider("Top-K neighbors", 10, 500, 200, 5)
        min_similarity = st.slider("Minimum similarity", 0.0, 8.0, 3.0, 0.1)

    count_errors_as_elims = st.checkbox(
        "Count expression errors as eliminations", value=False,
        help="If ON, errors count as eliminating the winner; if OFF, errors are excluded from the denominator."
    )
    ...
    if not hist_file:
        st.error("Upload a history file."); return
    if not filt_file:
        st.error("Upload a filters CSV."); return

    # ---------- Load history robustly ----------
    try:
    ...
    cols_lower_map = {c.lower(): c for c in df_hist.columns}
    seed_col = next((cols_lower_map[k] for k in ["seed","prev","previous","prev_seed","seed_value"] if k in cols_lower_map), None)
    winner_col = next((cols_lower_map[k] for k in ["winner","result","current","draw","win_value","result_value"] if k in cols_lower_map), None)

    if seed_col is None and winner_col is None:
        # One-column results or Date+Result
        if len(df_hist.columns) == 1:
            winner_col = df_hist.columns[0]
            df_hist.rename(columns={winner_col: "Result"}, inplace=True)
            winner_col = "Result"
        else:
            winner_col = cols_lower_map.get("result") or cols_lower_map.get("draw")

    if winner_col is None:
        st.error(f"History must include a results column (e.g., 'Result'). Found: {list(df_hist.columns)}")
        return

    if seed_col is None:
        series = df_hist[winner_col].astype(str).tolist()
        if len(series) < 2:
            st.error("History needs at least two rows to generate seeds from results.")
            return
        if reverse_hint:
            # newest->oldest
            gen_seed = series[1:]; gen_win = series[:-1]
        else:
            # oldest->newest
            gen_seed = series[:-1]; gen_win = series[1:]
        df_hist = pd.DataFrame({"seed": gen_seed, "winner": gen_win})
    else:
        df_hist = df_hist[[seed_col, winner_col]].rename(columns={seed_col:"seed", winner_col:"winner"})
    ...
    if nbrs.empty:
        st.warning("No historical neighbors met the threshold. Consider reducing Minimum similarity or increasing Top-K.")
        return
    ...
    # Optional enabled column restriction
    if use_enabled_col and ("enabled" in df_filters.columns):
        df_filters = df_filters[df_filters["enabled"].astype(str).str.lower().isin(["true","1","yes","y","t"])].copy()

    # Optional Filter ID restriction
    include_ids = set()
    if limit_to_ids:
        if pasted_ids.strip():
        ...
        if ids_from_csv is not None:
            try:
                ...
                lowmap = {c.lower(): c for c in df_ids.columns}
                idname = lowmap.get("id")
                if idname:
                    include_ids.update(df_ids[idname].astype(str).str.strip().tolist())
            except Exception as e:
                st.warning(f"Could not read IDs CSV: {e}")
        if include_ids:
            df_filters = df_filters[df_filters["id"].astype(str).isin(include_ids)].copy()
        else:
            st.warning("Limit to IDs is enabled but no IDs were parsed; evaluating all filters.")

    # ---------- Evaluate historical safety (percentages) ----------
    results = []
    for _, f in df_filters.iterrows():
        ...
        for _, row in nbrs.iterrows():
            ...
            context = {
                ...
                # aliases for legacy expressions
                "result": win_d, "result_digits": win_d, "digits": win_d, "draw_digits": win_d,
                "win_digits": win_d, "w_digits": win_d, "res_digits": win_d,
                "seed": seed_d, "prev_digits": seed_d, "winner": win_d, "current_digits": win_d,
            }
            try:
                res_bool = HFR_safe_eval_expression(expr, context)
                valid += 1
                if res_bool:
                    eliminated += 1
            except Exception:
        ...
        denom = valid if not count_errors_as_elims else (valid + errors)
        elim_rate = (eliminated / denom) if denom else 0.0
        safety = 1.0 - elim_rate if denom else 0.0
        results.append({
            "id": fid, "name": fname,
            "valid_cases": valid, "error_cases": errors,
            "elimination_rate_%": round(elim_rate*100.0, 2),
...
try:
    ...
    # If Streamlit not imported in base or during non-app context, no-op.
    pass
