...
# ========== ADDITIVE: Historical Safety — Two-Step (Same Page, Base Semantics) ==========
# Mirrors base behavior: an expression eliminates when bool(eval(expr)) is True.
import streamlit as st
import pandas as pd
import re
...
def _h3_digits(s: str): return digits_of(s)

def _h3_profile(digs):
...
def _h3_similarity(p_now, p_hist, w=None):
...
def _h3_eval_truthy(expr_code, winner_digits, base_env):
    env = combo_env(base_env, "".join(str(d) for d in winner_digits))
    globs={"__builtins__":{}}; globs.update(env)
    try:
        val = eval(expr_code, globs, env)
        return True, bool(val), None  # <- base semantics: truthy counts
    except Exception as e:
        return False, False, f"{type(e).__name__}: {e}"

def _render_two_step_v3():
    with st.sidebar:
        st.markdown("---")
        show = st.checkbox("Historical Safety — Two-Step (base semantics)", value=True)
    if not show: return

    st.markdown("## Historical Safety — Two-Step (base semantics)")

    c1,c2=st.columns(2)
    with c1:
        hist_file = st.file_uploader("History CSV/TXT", type=["csv","txt"], key="h3_hist")
        reverse = st.checkbox("History is reverse chronological (newest first)", value=True, key="h3_rev")
    with c2:
        seed_txt = st.text_input("Current seed (5 digits)", "", key="h3_seed")

    if not seed_txt or len(_h3_digits(seed_txt))!=5:
        st.warning("Enter a 5-digit seed to proceed."); return
    seed_now=_h3_digits(seed_txt); prof_now=_h3_profile(seed_now)

    with st.expander("Similarity settings", expanded=False):
        w={"sum":st.slider("Weight: Sum",0.0,3.0,1.0,0.1,key="h3_wsum"),
           "sum_category":st.slider("Weight: Sum Category",0.0,3.0,1.5,0.1,key="h3_wsumcat"),
           "even":st.slider("Weight: Even",0.0,3.0,0.6,0.1,key="h3_wev"),
           "odd":st.slider("Weight: Odd",0.0,3.0,0.6,0.1,key="h3_wod"),
           "high":st.slider("Weight: High",0.0,3.0,0.8,0.1,key="h3_whi"),
           "low":st.slider("Weight: Low",0.0,3.0,0.8,0.1,key="h3_wlo"),
           "spread":st.slider("Weight: Spread",0.0,3.0,0.8,0.1,key="h3_wsp"),
           "vtrac":st.slider("Weight: V-Trac Overlap",0.0,3.0,1.2,0.1,key="h3_wvtr")}
        K=st.slider("Top-K neighbors",10,500,200,5,key="h3_k")
        thr=st.slider("Minimum similarity",0.0,8.0,3.0,0.1,key="h3_thr")

    if st.button("Step 1 — Lock similar seeds", key="h3_s1"):
        if not hist_file:
        ...
        dfh["seed_digits"]=dfh["seed"].apply(_h3_digits)
        dfh["winner_digits"]=dfh["winner"].apply(_h3_digits)
        ...
        dfh["similarity"]=[_h3_similarity(_h3_profile(seed_now),{"sum":r.p_sum,"sum_category":r.p_sumcat,"even":r.p_even,"odd":r.p_odd,"high":r.p_high,"low":r.p_low,"vtrac":r.p_vtr,"spread":r.p_spread},w) for _,r in dfh.iterrows()]
        ...
        st.session_state["H3_NEIGHBORS"]=nbrs[["seed","winner","seed_digits","winner_digits","similarity"]].copy()
        st.success(f"Locked {len(st.session_state['H3_NEIGHBORS'])} similar seeds.")
        st.dataframe(st.session_state["H3_NEIGHBORS"].head(50), hide_index=True, use_container_width=True)

        wins_df=st.session_state["H3_NEIGHBORS"][["winner"]].rename(columns={"winner":"winner_draw"}).copy()
    ...
    filt_file = st.file_uploader("Filters CSV (id + expression required)", type=["csv"], key="h3_filters")
    limit_ids=st.checkbox("Limit to pasted Filter IDs", value=False, key="h3_limit_ids")
    ids_txt=st.text_area("Paste IDs (comma/space/newline separated)", "", height=100, key="h3_ids")

    if st.button("Evaluate on locked winners", key="h3_s2"):
        if "H3_NEIGHBORS" not in st.session_state:
        ...
        # Compile with base normalizer
        comp=[]
        for _,r in dff.iterrows():
            code,norm=_h3_compile(str(r["expression"]))
            comp.append({"id":str(r["id"]), "name":str(r["name"]), "code":code, "norm":norm})
        comp=pd.DataFrame(comp)

        # Base env (same as app)
        base_env = make_base_env("".join(str(d) for d in seed_now), "", "", "",
                                 st.session_state.get("hot_digits", []),
                                 st.session_state.get("cold_digits", []),
                                 st.session_state.get("due_digits", []))

        nbrs=st.session_state["H3_NEIGHBORS"]
        results=[]
        for _, fr in comp.iterrows():
            fid,fname,code=fr["id"],fr["name"],fr["code"]
            valid=err=elim=0
            for _, n in nbrs.iterrows():
                ok, val, err_msg = _h3_eval_truthy(code, n["winner_digits"], base_env)
                if ok:
                    valid+=1
                    if val: elim+=1
                else:
